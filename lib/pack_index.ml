...
let lengths offsets =
  ...
  let l = List.sort ~cmp:(fun (_,x) (_,y) -> Int.compare x y) offsets in
  aux [] l
...
let input buf =
  ...
  let crcs =
    ...
    for i=0 to nb_objects-1 do
      a.(i) <- (names.(i), Mstruct.get_be_uint32 buf);
    done;
    Array.sort ~cmp:(fun (k1,_) (k2,_) -> SHA1.compare k1 k2) a;
    a in
  ...
  let offsets, conts =
    ...
    for i=0 to nb_objects-1 do
      if Int.((Mstruct.get_uint8 buf) land 128 <> 0) then (
        b.(i) <- true;
  ...
  let crcs = SHA1.Map.of_sorted_array_unchecked crcs in
  { offsets; lengths; crcs; pack_checksum }
...
let add buf t =
  ...
  let cmp (k1,_) (k2,_) = SHA1.compare k2 k1 in
