...
module Log = (val Git_misc.src_log "index" : Logs.LOG)
...
let compare_entries e1 e2 =
  ...
  | 0 -> Git_hash.Blob.compare e2.id e1.id
  | i -> i
...
let err_wrong_checksum ~got ~expected =
  fail "Wrong checksum! got %a but was expecting %a."
    Git_hash.pp got Git_hash.pp expected
...
module IO (D: Git_hash.DIGEST) = struct
  ...
  let input_entry buf =
    ...
    let id = Git_hash_IO.input buf |> Git_hash.to_blob in
    let stage, len =
    ...
    Log.debug (fun l ->
        l "name:%s id:%a bytes:%d padding:%d" name Git_hash.Blob.pp id bytes padding
      );
    { stats; id; stage; name }
  ...
  let add_entry buf t =
    ...
    Mstruct.with_mstruct cstr (fun mstr ->
        ...
        Mstruct.set_string mstr (Git_hash.Blob.to_raw t.id);
  ...
  let input buf =
    ...
    let expected = Git_hash_IO.input buf in
    if not (Git_hash.equal got expected) then err_wrong_checksum ~got ~expected;
    { entries; extensions }

  let add buf ?level:_ t =
    let str = Git_misc.with_buffer (fun buf ->
    ...
    Buffer.add_string buf (Git_hash.to_raw h)
