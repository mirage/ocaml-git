...
let mimic_git_conf ~edn () =
  ...
  impl @@ object
       ...
       method! connect _ modname _ =
         Fmt.str
           "Lwt.return (%s.with_resolv (%s.with_smart_git_endpoint (%a) \
            %s.ctx))"
...
let mimic, is_ssh = mimic ~edn:remote ~kind:`Rsa ~seed:ssh_seed ~auth:ssh_auth

let () =
  register "minigit" ~packages:[]
    [ minigit $ git $ time $ console $ resolver $ conduit $ mimic $ is_ssh ]
