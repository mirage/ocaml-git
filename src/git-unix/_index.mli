...
module Make
    (Store : Git.S)
    (FS : Git.FS)
    (Entry : ENTRY with type hash := Store.Hash.t) : sig
  module Store : Git.S
  ...
  val load_entries :
    Store.t -> FS.t -> dtmp:Cstruct.t -> (Entry.entry list, error) result Lwt.t
  ...
  module Write : sig
    ...
    val update_on_store : Store.t -> FS.t -> (Store.Hash.t, error) result Lwt.t
  end
  ...
  module Snapshot : sig
    val of_tree :
      Store.t -> FS.t -> Store.Hash.t -> (Entry.entry list, error) result Lwt.t
