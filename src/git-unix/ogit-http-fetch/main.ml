  ...
  let references =
    List.fold_left
      (fun references (remote_ref, local_ref) ->
        try
          let local_refs = Store.Reference.Map.find remote_ref references in
          if List.exists (Store.Reference.equal local_ref) local_refs then
            references
          else
            Store.Reference.Map.add remote_ref (local_ref :: local_refs)
              references
        with Not_found ->
          Store.Reference.Map.add remote_ref [local_ref] references )
      Store.Reference.Map.empty references
  in
  Store.v root
  >>!= store_err
  >>?= fun git ->
  Sync.Http.fetch_some git ~references (Sync.Http.endpoint repository)
  >>!= sync_err
  >>?= fun _ -> Lwt.return (Ok ())
...
module Flag = struct
  ...
  let references =
    ...
    Arg.(
      non_empty
      & pos_all (pair ~sep:':' reference reference) []
      & info [] ~doc ~docv:"<ref>")
end
