...
struct
        ...
        ->
          let pp_entry ppf { untracked; subdirectory; name } =
            Fmt.pf ppf
              ...
              untracked subdirectory name in
          Fmt.pf ppf
  ...
  type t = {
    ...
    hash : Hash.ctx;
    extensions : Ext.value list;
    (* XXX(dinosaure): should be a set. *)
    state : state;
  }
  ...
  let pp_state ppf = function
    ...
    | Extension (rest, ext) ->
        Fmt.pf ppf "(Extension (rest:%d, %a))" rest (Fmt.hvbox pp_extension) ext
  ...
  let rec get_byte ?(digest = true) ~ctor k src t =
    ...
    then
      ...
      let t' =
        {
          t with
          state = ctor (fun src t -> (get_byte [@tailcall]) ~ctor k src t);
        } in
  ...
  let rec get_lsb32 ?(digest = true) ~ctor k src t =
    ...
    then
      ( get_byte @@ fun byte0 ->
        get_byte @@ fun byte1 ->
        get_byte @@ fun byte2 ->
        get_byte @@ fun byte3 src t ->
        k (from_lsb32 byte0 byte1 byte2 byte3) src t )
        src t
    else
      let t' =
        {
  ...
  module KEntry = struct
    ...
    let get_extend k src t =
      ( get_lsb16 @@ fun extend src t ->
        match (extend land 0x8000, extend land 0x4000, extend land 0x2000) with
        | reserved, skip_worktree, intent_to_add ->
            k
              {
  ...
  module KExtension = struct
    ...
    let get_digit ~ext k rest src t =
      ...
      let rec go rest src t =
        ( peek_byte @@ fun byte rest src t ->
          match Option.map Char.unsafe_chr byte with
          | Some ('0' .. '9' as chr) ->
              Buffer.add_char buf chr ;
              go (rest - 1) src
                { t with i_pos = t.i_pos + 1; read = t.read + 1 }
    ...
    let get_number ~ext k rest src t =
      ...
      ( peek_byte @@ fun byte ->
        ...
        | Some '-' ->
            get_byte @@ fun _ ->
            get_digit @@ fun n rest src t -> k (-n) rest src t
                ...
                k
                  {
                    ...
                    stages = [ (mode0, hash0); (mode1, hash1) ];
                  }
            | mode0, mode1, mode2 ->
                get_hash @@ fun hash0 ->
                get_hash @@ fun hash1 ->
  ...
  let path info hash flag src t =
    ...
    | Some previous_name ->
        ( KEntry.get_varint @@ fun len' ->
          let lead =
            String.sub previous_name 0 (String.length previous_name - len')
          in
          KEntry.to_nul @@ fun rest _ t ->
          let entry =
            { Entry.info; hash; flag; path = Git.Path.v (lead ^ rest) } in
    ...
    | None ->
        ...
        then
          ( KEntry.to_nul @@ fun name _ t ->
            ...
            let padding = if t.read mod 4 = 0 then 0 else 4 - (t.read mod 4) in
            Cont
              { t with entries = entry :: t.entries; state = Padding padding }
          )
            src t
        else
          ( KEntry.take flag.length @@ fun name _ t ->
  ...
  let entry src t =
    ( KEntry.get_time @@ fun ctime ->
      KEntry.get_time @@ fun mtime ->
      KEntry.get_lsb32 @@ fun dev ->
      KEntry.get_lsb32 @@ fun ino ->
      KEntry.get_kind @@ fun mode ->
      KEntry.get_lsb32 @@ fun uid ->
      KEntry.get_lsb32 @@ fun gid ->
      KEntry.get_lsb32 @@ fun size ->
      KEntry.get_hash @@ fun hash ->
      KEntry.get_flag @@ fun flag ->
      ...
      | Some false | None ->
          path info hash
            { flag with Entry.extend = (None : Entry.extend option) } )
      src t
  ...
  let rec signature src t =
    let get_signature src t =
      ( KSignature.get_byte @@ fun byte0 ->
        KSignature.get_byte ~digest:false @@ fun byte1 ->
        KSignature.get_byte ~digest:false @@ fun byte2 ->
        KSignature.get_byte ~digest:false @@ fun byte3 src t ->
        Log.debug (fun l ->
  ...
  and move length src t =
    ...
    | n ->
        ...
        then
          ...
          move (length - n') src
            { t with i_pos = t.i_pos + n'; read = t.read + n' }
  ...
  and move_or_die a b src t =
    ...
    let final consumed src t =
      ...
      then
        Cont
          {
            t with
            state = Hash (Hash.of_raw_string (Cstruct.to_string consumed));
          }
      else
    ...
    let rec consume_to_hash consumed src t =
      ...
      then
        ...
        (* has > expect_if_hash *)
        let hash = Hash.feed_cstruct t.hash consumed in
        move (Int32.to_int expect_if_extension) src { t with hash } in
    await src
      {
        t with
        i_pos = t.i_pos + (t.i_len - t.i_pos);
  ...
  module KEntry = struct
    ...
    let put_varint len k dst t =
      ...
      let rec loop idx dst t =
        ...
        then k dst t
          ...
          put_byte byte (loop (idx + 1)) dst t in
      loop !pos dst t
    ...
    let put_string str k dst t =
      let rec go pos dst t =
        ...
        else put_byte (Char.code str.[pos]) (go (pos + 1)) dst t in
      go 0 dst t
...
struct
  ...
  let load fs ~root ~dtmp =
    ...
    (FS.File.open_r fs Fpath.(root / "index") [@warning "-44"]) >>= function
    | Error sys_err ->
        Log.debug (fun l ->
    ...
    | Ok read ->
        ...
        let rec loop decoder =
          ...
          | `End (_, index, extensions) ->
              FS.File.close read >>= fun _ ->
              Lwt.return (Ok (index, extensions))
          | `Await decoder -> (
              ...
              | Ok n ->
                  ...
                  loop (IDec.refill 0 n decoder)) in
        loop decoder
...
struct
  ...
  let chain_of_entry path entry =
    ...
    List.fold_right
      ...
      segs (Blob entry)
    |> function
    | Blob _ -> Fmt.invalid_arg "Path is empty"
    | Tree map -> map
  ...
  let to_entries ~to_entry (Root t) =
    let rec aux path name e acc =
      ...
      | Tree tree -> StringMap.fold (aux Git.Path.(path / name)) tree acc in
  ...
  let lwt_result_traversal acc fblob ftree (Root map) =
    ...
    let rec go acc =
      ...
      | path, Tree sub ->
          ...
          >>= fun () -> go acc
      | exception Queue.Empty -> Lwt.return (Ok acc) in
    go acc
  ...
  let pp_stats ppf stats =
    ...
    let pp_octal ppf n =
      ...
      loop (fun () -> ()) n in
    Fmt.pf ppf
      "{ @[<hov>dev = %d;@ ino = %d;@ kind = %a;@ perm = %a;@ nlink = %d;@ uid \
       = %d;@ gid = %d;@ rdev = %d;@ size = %d;@ atime = %f;@ mtime = %f;@ \
       ctime = %f;@] }"
  ...
  let entry_of_stats hash path ?(gitlink = false)
    ...
    let info =
      ...
      {
        ctime =
          { lsb32 = Int32.of_float st_ctime; nsec = Int32.of_float st_ctime };
        mtime =
          { lsb32 = Int32.of_float st_mtime; nsec = Int32.of_float st_mtime };
    ...
    let flag =
      ...
      let length =
        ...
        min 0xFFF n in
      { assume = false; extend = None; stage = 0; length } in
  ...
  let write_blob ~root fs path ((perm : Store.Value.Tree.perm), raw) =
    ...
    | #file as perm ->
        FS.File.open_w fs abs_path >!= file_err >?= fun fd ->
        let rec loop ~retry raw =
          if Cstruct.len raw = 0
          then Lwt.return (Ok ())
          else
            FS.File.write raw fd >!= file_err >?= function
            | 0 when retry >= 50 ->
                Lwt.return Git.Error.(v @@ FS.err_stack abs_path)
            | 0 -> loop ~retry:(retry + 1) raw
            | len -> loop ~retry:0 (Cstruct.shift raw len) in
        Lwt.catch
          (fun () ->
            Lwt.finalize
              ...
              (fun result ->
                Log.debug (fun l ->
                    l "Blob %a:%a wrote." Hash.pp
                      ...
                      Fpath.pp abs_path) ;
                Unix.chmod Fpath.(to_string abs_path) (int_of_perm perm) ;
                FS.File.close fd >>= function
                | Ok () -> Lwt.return result
                | Error err -> Lwt.fail (Close err)))
          (function
            | Close err ->
                Lwt.return (Error (Git.Error.FS.err_file abs_path err))
            | exn -> Lwt.fail exn)
        >?= fun () ->
        Unix.stat Fpath.(to_string abs_path) |> fun stats ->
        Lwt.return_ok
          (entry_of_stats Store.Value.Blob.(digest (of_cstruct raw)) path stats)
    | #directory ->
        Fmt.invalid_arg "Unable to make a new directory from a Git blob object."

  let write_blob_with_acc ~root fs acc path tree_entry =
    write_blob ~root fs path tree_entry >|= function
  ...
  let write_blob_entry git fs path (entry : Entry.entry) =
    ...
    | Store.Value.Blob raw ->
        write_blob ~root:(Store.root git) fs path
  ...
  let write_blob_entry_with_acc git fs acc path entry =
    write_blob_entry git fs path entry >|= function
    | Ok entry -> Ok (entry :: acc)
    | Error _ as err -> err
  ...
  let index_to_store git fs ~dtmp =
    ...
    lwt_result_traversal []
      ...
      (write_tree ~root:(Store.root git) fs)
      (of_entries entries)
  ...
  module Write = struct
    ...
    let blob_of_path t path =
      ...
      then
        ...
        let link =
          Astring.String.map (function '\\' -> '/' | chr -> chr) link in
        ...
        let ic = open_in Fpath.(to_string abs_path) in
  ...
  module Read = struct
    ...
    let of_tree git hash =
      make_from_tree ~path:Git.Path.root git hash >>= fun (Root t) ->
  ...
  module Snapshot = struct
    let of_tree git fs hash =
      make_from_tree ~path:Git.Path.root git hash >>= fun root ->
      lwt_result_traversal []
