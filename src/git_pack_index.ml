...
module Log = (val Git_misc.src_log "pack-index" : Logs.LOG)
...
module Raw = struct
  ...
  let compare t1 t2 =  Git_hash.compare t1.pack_checksum t2.pack_checksum
  let equal t1 t2 = Git_hash.equal t1.pack_checksum t2.pack_checksum

  let pp ppf t =
    Format.fprintf ppf "@[pack-checksum: %a@ " Git_hash.pp t.pack_checksum;
    let l = ref [] in
    let offsets = Git_hash.Map.to_alist t.offsets in
    let crcs = Git_hash.Map.to_alist t.crcs in
    List.iter2 (fun (key1, offset) (key2, crc) ->
        assert (key1 = key2);
        l := (key1, offset, crc) :: !l
      ) offsets crcs;
    let l = List.sort (fun (s1,_,_) (s2,_,_) -> Git_hash.compare s1 s2) !l in
    List.iter (fun (h, offset, crc) ->
        Format.fprintf ppf "@[%a@ off:%d@ crc:%ld@;@]" Git_hash.pp h offset crc
      ) l;
    Format.fprintf ppf "@]"
  ...
  let lengths { offsets; _ } =
    ...
    let l = Git_hash.Map.bindings offsets in
    let l = List.sort (fun (_,x) (_,y) -> Pervasives.compare x y) l in
    Git_hash.Map.of_alist (aux [] l)

  let keys t = Git_hash.Set.of_list (Git_hash.Map.keys t.offsets)

  let find_offset t h =
    try Some (Git_hash.Map.find h t.offsets)
    with Not_found -> None
...
module Offset_cache = struct
  type t = int Git_lru.t
  let create size = Git_lru.make size
  let add = Git_lru.add
  let find = Git_lru.find
end
...
module Make (D: Git_hash.DIGEST) = struct

  module A = Git_hash.Array(D)
  ...
  let get_hash_idx t h =
    Log.debug (fun l -> l "get_hash_idx: %a" Git_hash.pp h);
    ...
    let offset, len =
      if fanout_idx = 0 then 0, get_int offsets
        ...
        fail "get_hash_idx %a" Git_hash.pp h;
    in
    let buf = hashes t in
  ...
  let find_offset t h =
    Log.debug (fun l -> l "find_offset: %a" Git_hash.pp h);
  ...
  let mem t h =
    Log.debug (fun l -> l "mem: %a" Git_hash.pp h);
  ...
  module Raw = struct
    ...
    include Raw
    module Git_hash_IO = Git_hash.IO(D)

    let input_keys buf n =
      Log.debug (fun l -> l "input: reading the %d objects IDs" n);
      let a = Array.make n (Git_hash.of_raw "") in
      for i=0 to n - 1 do
        a.(i) <- Git_hash_IO.input buf;
      done;
      a
    ...
    let input buf =
      ...
      let crcs =
        let a = Array.make nb_objects (Git_hash.of_raw "", 0l) in
      ...
      let pack_checksum = Git_hash_IO.input buf in
      let _checksum = Git_hash_IO.input buf in

      let offsets_alist = Array.to_list offsets in
      let offsets = Git_hash.Map.of_alist offsets_alist in
      let crcs = Git_hash.Map.of_alist (Array.to_list crcs) in
      { offsets; crcs; pack_checksum }

    let add buf ?level:_ t =
      let n = Git_hash.Map.cardinal t.offsets in
      Log.debug (fun l -> l "output: %d packed values" n);
      Buffer.add_string buf "\255tOc";
      Git_misc.add_be_uint32 buf 2l;

      let cmp (k1,_) (k2,_) = Git_hash.compare k1 k2 in

      let offsets = List.sort cmp (Git_hash.Map.to_alist t.offsets) in
      let crcs    = List.sort cmp (Git_hash.Map.to_alist t.crcs) in

      Log.debug (fun l -> l "output: writing the first-level fanout");
      let fanout = Array.make 256 0l in
      List.iter (fun (key, _) ->
          let str = Git_hash.to_raw key in
          let n = Char.code str.[0] in
          for i = n to 255 do
            fanout.(i) <- Int32.succ fanout.(i)
          done;
        ) offsets;
      Array.iter (Git_misc.add_be_uint32 buf) fanout;

      Log.debug (fun l -> l "output: writing the %d object IDs" n);
      List.iter (fun (key, _) ->
          Git_hash_IO.add buf key
        ) offsets;

      Log.debug (fun l -> l "output: writing the %d CRCs" n);
      List.iter (fun (_, crc) ->
          Git_misc.add_be_uint32 buf crc
        ) crcs;
      ...
      List.iter (fun (_, offset) ->
          if offset <= Int32.(to_int max_int) then (
            ...
            Git_misc.add_be_uint32 buf i
          ) else (
            conts := Int64.of_int offset :: !conts;
            Git_misc.add_be_uint32 buf 0x80_00_00_00l
          )
        ) offsets;
      ...
      (* XXX: Git_hash.of_bigstring *)
      let str = Buffer.contents buf in
      let checksum = D.string str in
      Buffer.add_string buf (Git_hash.to_raw checksum)
