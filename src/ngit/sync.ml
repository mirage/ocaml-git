    ...
    *)
    ...
    let canonicalize_pack git info =
      ...
      let make acc (hash, (crc, off)) =
        ...
        | Error err ->
          Format.printf "impossible to get: %a -> %a\n%!" Hash.pp hash Decoder.pp_error err;
          Lwt.fail (Leave err)
        | Ok o ->
      ...
      let plus acc =
        Lwt_list.fold_left_s
          (fun acc hash ->
            ...
            | None ->
              Format.printf "Impossible to get the external ressource: %a\n%!" Hash.pp hash;
      ...
      | Error (PACKEncoder.Delta.Invalid_hash hash) ->
        Lwt.fail (Leave (Decoder.Invalid_hash hash))
      | Ok entries ->
        Format.printf "delta-ification finished\n%!";
  ...
  let push_handler git ~push ~packer t r =
    ...
    let rec pack src state t r =
      let rec go src dst state t =
        Format.printf "src> @[<hov>%a@]\n%!"
          Cstruct.hexdump_pp (option_value ~default:empty src);
        Format.printf "dst> @[<hov>%a@]\n%!"
          Cstruct.hexdump_pp dst;
