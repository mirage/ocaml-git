...
module Make (Uid : UID) = struct
  ...
  let unix_store_appendv root uid payloads =
    ...
    let payloads =
      List.map
        (fun payload -> (payload, 0, Bigstringaf.length payload))
        payloads in
    let process () =
      Lwt_unix.openfile (Fpath.to_string path) Unix.[ O_WRONLY ] 0o644
      >>= fun fd ->
      let rec go = function
        ...
        | (payload, off, len) :: rest ->
            ...
            if len - len' = 0
            then go rest
            else go ((payload, off + len', len - len') :: rest) in
      go payloads in
    let error = Lwt.fail in
    Lwt.catch process error
  ...
  let cut ~sep:({ Cstruct.len = sep_len; _ } as sep) ({ Cstruct.len; _ } as t) =
    ...
    let rec check_sep i k =
      ...
      else if Cstruct.get_char t (i + k) = Cstruct.get_char sep k
      then check_sep i (succ k)
      else scan (succ i)
    and scan i =
      if i > max_t_zidx
      then None
      else if Cstruct.get_char t i = Cstruct.get_char sep 0
      then check_sep i 1
      else scan (succ i) in
    scan 0
  ...
  let hdr_get raw =
    ...
    | Some (length, contents) ->
        ...
        let kind =
          ...
          | "tag" -> `D
          | "tree" -> `B
          | v -> Fmt.failwith "Invalid type of Git object: %s" v in
