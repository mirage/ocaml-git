...
let to_string node =
  let hex = Git_hash.to_hex node in
  String.sub hex 0 8

module C =
  Graph.Imperative.Digraph.ConcreteBidirectionalLabeled
    (struct
      type t = (Git_hash.t * Git_value.t)
      let compare (x,_) (y,_) = Git_hash.compare x y
      let hash (x,_) = Git_hash.hash x
      let equal (x,_) (y,_) = Git_hash.equal x y
    end)
    (struct
      type t = string
      let default = ""
...
module Dot = Graph.Graphviz.Dot(struct
    ...
    let vertex_name (x,o) =
      ...
      let p = match o with
        | Git_value.Blob _   -> "B"
        | Git_value.Commit _ -> "C"
        | Git_value.Tree _   -> "Tr"
        | Git_value.Tag  _   -> "Ta" in
      Printf.sprintf "\"%s-%s\"" p hex
    let vertex_attributes (_,o) =
      match o with
      | Git_value.Commit _ -> [`Shape `Doublecircle]
      | _ -> []
...
module Make (Store: Git_store.S) = struct
  ...
  let closure ?(full=true) t ~min ~max =
    ...
    let min = Git_hash.Set.to_list min in
    ...
    let rec add key =
      if has_mark key then Lwt.return ()
        ...
        mark key;
        Log.debug (fun l -> l "ADD %a" Git_hash.pp key);
    ...
    let max = Git_hash.Set.to_list max in
