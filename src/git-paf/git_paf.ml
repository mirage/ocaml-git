...
let get ~ctx ?(headers = []) uri : (_, error) result Lwt.t =
  ...
  Format.eprintf ">>> GET %a.\n%!" Uri.pp uri;
  let f uri = Paf_cohttp.get ~ctx ~headers uri in
...
let post ~ctx ?(headers = []) uri body : (_, error) result Lwt.t =
  ...
  Format.eprintf ">>> POST %a.\n%!" Uri.pp uri;
  let f uri = Paf_cohttp.post ~ctx ~headers ~chunked:false ~body uri in
  with_redirects ~f uri >>= fun (_resp, body) ->
  Cohttp_lwt.Body.to_string body >>= fun body -> Lwt.return_ok ((), body)
...
module Make
    ...
    (Paf : Paf.S with type stack = Stack.t) (TCP : sig
