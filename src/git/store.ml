...
module FS (H: S.HASH) (F: S.FS) (I: S.INFLATE) (D: S.DEFLATE) = struct
  ...
  module Pack = struct
    ...
    let to_temp_file fmt stream =
      ...
      let rec go ?chunk ~call () =
        ...
        | Some raw ->
          ...
          | Error err -> Lwt.return Error.(v @@ ErrFS.err_write path err)
          | Ok 0 when len <> 0 ->
            if call <> 50 (* XXX(dinosaure): as argument? *)
            then go ?chunk ~call:(call + 1) ()
            else Lwt.return Error.(v @@ ErrFS.err_stack path)
          | Ok n when n = len ->
    ...
    let canonicalize git path_pack decoder_pack fdp ~htmp ~rtmp delta info =
      ...
      | Ok entries ->
        ...
        | Ok (path, sequence, hash_pack) ->
          (PackImpl.save_idx_file ~root:git.dotgit sequence hash_pack >>!= lift_error) >>= function
          | Error _ as err -> Lwt.return err
          | Ok () ->
            ...
            (FS.File.move path Fpath.(git.dotgit / "objects" / "pack" / filename_pack) >>= function
              | Error err ->
                Lwt.return Error.(v @@ ErrFS.err_move path Fpath.(git.dotgit / "objects" / "pack" / filename_pack) err)
    ...
    let from git stream =
      ...
      | Error err ->
        Lwt.return Error.(v @@ ErrFS.err_open path err)
      | Ok fdp ->
        ...
        >>= function
        | Error err ->
          Lwt.return Error.(v @@ ErrFS.err_length path err)
        | Ok decoder ->
          ...
          then
            ...
            then begin
              ...
              (FS.Mapper.close fdp >>!= ErrFS.err_close path)
              >>?= fun () ->
  ...
  let indexes git =
    ...
    | Error err ->
      Lwt.return Error.(v @@ ErrFS.err_sys_dir err)
  ...
  module Ref = struct
    ...
    let mem t reference =
      ...
      | false -> let v = in_packed_refs () in Lwt.return v
  ...
  let create ?root ?dotgit ?(compression = 4) ?buffer () =
    ...
    | None, _ | _, None ->
      (FS.Dir.current () >>!= ErrFS.err_sys_dir)
      >>?= fun current ->
      ...
      >>?= (fun engine ->
          Lwt.return (Ok { dotgit
                         ...
                         ; cache = cache ()
                         ; buffer = buffer }))
      >>?= (fun git -> (sanitize_head git
                        >>?= fun () -> Lwt.return (Ok git)))
