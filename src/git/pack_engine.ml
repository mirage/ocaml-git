...
= struct
  ...
  let merge t info =
    ...
    let hunks', depth' =
      match buffers.depth < depth,
            hunks_length < info.Pack_info.max_length_insert_hunks with
      | false, true ->
  ...
  let add_total ~root t path info =
    let `Full { Pack_info.Full.hash = hash_pack; Pack_info.Full.thin; } = info.Pack_info.state in
  ...
  let rec read_and_exclude ~root t exclude request =
    (* XXX(dinosaure): [read_and_exclude] is called when a pack
       decoder wants to reconstruct an internal object and the source
       is external - by this way, we can consider the pack file as a
       /thin/ pack.
       ...
       However, from a [ `Partial ] information, we can not ensure
       than the pack file is not a /thin/-pack. So this function
       exists for the pack decoder when we want to retrieve an
       external object.

       Then, we exclude the pack itself to avoid a infinite recursion
       when the length of the redirection is 1. If it's upper,
       firstly, what? then, you could have a infinite loop. About this
       bug, we could change the API of the pack decoder and allow to
       inform a /close/ list to the function. *)
  ...
  let force_total t loaded =
    let `Partial { Pack_info.Partial.hash = hash_pack; Pack_info.Partial.delta; } = loaded.info.Pack_info.state in
    ...
    Lwt_list.map_s
      (fun (offset, hunks_descr) ->
         ...
         | Error err ->
           Log.err (fun l -> l ~header:"force_total" "Retrieve an error when we try to reconstruct \
    ...
    >>= Lwt_list.fold_left_s
      (fun ((tree, graph) as acc) -> function
    ...
    then
      ...
      Lwt.return (Ok (loaded.decoder,
                      ...
                      { loaded.info with Pack_info.tree = tree'
                                       ...
                                       ; Pack_info.state = `Full { Pack_info.Full.hash = hash_pack
                                                                 ; Pack_info.Full.thin = not is_not_thin } }))
    else Lwt.return (Error (`Integrity (Fmt.strf "Impossible to get all informations from \
                                                  the pack file: %a." Hash.pp hash_pack)))
    ...
    Graph.fold
      (fun _ -> function
         ...
         | Exists { idx; _ } ->
           IDXDecoder.fold idx (fun hash _ acc -> hash :: acc)
         | Total { info; _ } ->
           fun acc -> Pack_info.Radix.fold (fun (hash, _) acc -> hash :: acc) acc info.Pack_info.tree)
      packs []
