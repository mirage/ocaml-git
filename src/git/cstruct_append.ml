...
let create { o0; o1; _ } key =
  let which, value =
    let k0 =
      Option.fold ~none:false
        ~some:(fun key' -> key == key')
        (Ephemeron.K1.get_key o0) in
    let k1 =
      Option.fold ~none:false
        ~some:(fun key' -> key == key')
        (Ephemeron.K1.get_key o0) in
    ...
    let value =
      if k0
      then Option.get (Ephemeron.K1.get_data o0)
      else Option.get (Ephemeron.K1.get_data o1) in
    (k1, value) in

  let value =
    if Cstruct.len !value < 1
    then (
...
let map _ fd ~pos len =
  ...
  if pos > fd.length
  then Lwt.return Bigstringaf.empty
  else
