...
module Make (D: Hash.DIGEST) (I: Inflate.S) = struct
  ...
  let write t value =
    ...
    if Hashtbl.mem t.values h then Lwt.return h
      ...
      Log.debug (fun l -> l "Writing %a" Hash.pp h);
  ...
  let err_write_pack expected got =
    let str =
      Fmt.strf
        "Git.Mem.write_pack: wrong checksum.\n\
  ...
  let rec read_reference t r =
    Log.debug (fun l -> l "Reading %a" Reference.pp r);
  ...
  let write_reference t r h =
    Log.debug (fun l -> l "Writing %a" Reference.pp r);
    Hashtbl.replace t.refs r (`H h);
    Lwt.return_unit

  let test_and_set_reference t r ~test ~set =
    Log.info (fun l -> l "Test and set: %a" Reference.pp r);
