...
sig
  ...
  (** [eval src t] is:
      ...
      {!continue} to move to the next step of the process, otherwise the
      decode sticks on this situation. The value will be consumed then. If
      the {!hunk} is [Insert], the internal [Cstruct.t] need to be copied
      because {!eval} will erase the content then.}

      {- [`Ok (t, hunks)] when [t] is done. We returns the complete value
      {!hunks}. Then, [t] sticks on this situation, the client can remove
      it.}
  ...
  (** Make a new decoder state {!t} from a {!reference}. We need to notice
      the length of the inflated stream to know when the decoder {!t} is
      done. *)

  val refill : int -> int -> t -> t
  (** [refill off len t] provides a new [t] with [len] bytes to read,
      starting at [off]. This byte range is read by calls to {!eval} with [t]
  ...
  val current : t -> hunk
  (** [current t] provides the current {!hunk} from the decoder [t]. This
      hunk is equivalent to: [eval _ t], which returns [`Hunk] with the same
      (physically) {!hunk}.
  ...
  (** [available_in t] returns how many byte is available in the current
      buffer noticed to the previous call of {!eval}. *)

  val read : t -> int
  (** [read t] returns how many byte [t] read at the beginning. The client
      can assert than [read t] is equal to [length] noticed when he {!make}
      the new decoder. *)
end
...
sig
  ...
  type error =
    ...
    | Invalid_kind of int
    (** Appears when the kind of the current PACK object is undefined. TODO:
        this case does not appear, lol ... *)
    | Inflate_error of Inflate.error
    (** Appears when the {!Inflate} module returns an error when it tries to
        inflate a PACK object. *)
    ...
    (** The Hunk object is length-defined. So, when we try to decode this
        specific PACK object, if the decoder {!H.t} expects some new bytes and
        we already get all input, this error appears (with respectively how
        many byte(s) we expected and how many byte(s) the decoder {!H.t}
        computed). *)
  ...
  type kind =
    ...
    (** The kind of the PACK object. It can be:
      ...
      {- A [Hunk]: this is not a git object but a specific PCK object. The
        ...
        just a result of a diff between the reference and the current object.}
        } *)

  val default : Cstruct.t -> Inflate.window -> t
  (** [default tmp window] makes a new decoder of a PACK file.

      [tmp] is a [Cstruct.t] used as an internal output of the inflated
      stream. We let the user to allocate the optimized length for this
      temporary buffer and we take the ownership (so, you don't use it to
      another compute).
  ...
  *)
  ...
  (** [from_window pack_window off_in_window tmp window] makes a new decoder
      of a PACK file specialized from a {!Window.t}. This decoder compute only
      one object in a precise offset (noticed by [pack_window] and the relative
  ...
  *)
  ...
  val next_object : t -> t
  (** [next_object t] provides a new [t] which continue the de-serialization
      of the PACK stream. Indeed, when the client catches a new [`Object], the
  ...
  val kind : t -> kind
  (** [kind t] returns the {!kind} of the current object. The client is able
      to use this function only when {!eval} returns [`Object] or [`Hunk].
  ...
  (** [eval src t] is:
      ...
      {- [`Await t] iff [t] more input storage. The client muse use {!refill}
      to provide a new buffer and then call {!eval} with [`Await] until other
      value returned.}

      {- [`Object t] iff [t] finishes to compute one object. The client is
      able to use the meta-data functions (like {!kind} or {!crc}) and should
      use {!next_object} to move the decoder to the next step. Otherwise, the
      decoder [t] sticks in this situation.}
      ...
      until [`Object] is returned. In this situation, we ensure than the future
      {!kind} of the current/expected object is different than the {!Hunk _}.}
...
sig
  ...
  type error =
    ...
    (** Appears when the result of the application of a {!P.H.hunks} returns a bad
       raw. *)
  ...
  sig
    type from =
      (** When the source is located in the PACK file. *)
      | Offset of { length   : int
                  (** Real inflated length of the object. *)
                  ; consumed : int
                  (** How many byte(s) consumed to de-serialize the object. *)
                  ; offset   : int64
                  (** The absolute offset in the PACK file of the object.
                     *)
                  ; crc      : Crc32.t
      ...
      | External of Hash.t
      (** When the object is not come from a source but directly serialized (no
         delta-ification) in the PACK file. *)
      | Direct of { consumed : int
                  (** How many byte(s) consumed to de-serialize the current
                     object. *)
                  ; offset   : int64
                  (** The absolute offset in the PACK file of the current
                     object. *)
                  ; crc      : Crc32.t
                  (** The CRC-32 checksum of the source object. *)
                  ; }
      (** The type to describe where the object come from (directly from the
         PACK file or from a delta-ification with an [External _] object or a
         {i in-PACK} object. *)
    and t =
      { kind   : kind
    ...
    (** [first_crc_exn t] gets the CRC-32 checksum of [obj] {!t}. This function
       can't raise an exception for any object created from this API. However,
       an object created by the hand could not respect the assertion. *)
  end

  val find : t -> int64 -> (Window.t * int, Mapper.error) result Lwt.t
  (** [find t absolute_offset] returns a couple of a {!Window.t} which contains
     the absolute offset requested and the relative offset in the window. We
     allocate a new {!Window.t} only when we don't find a previous valid
     {Window.t}. If the absolute offset is bad, we defer the exception from
     [Mapper.map]. *)
  ...
  val make : ?bucket:int -> Mapper.fd
    ...
    (** [make ?bucket fd cache idx revidx extern] makes a new decoder when:

      {ul

      {- [cache] is a function to return if it is available an object expected
       by the decoder.}

      {- [idx] is a function to return the absolute offset of the requested
       object if it is available in the PACK file.}

      {- [revidx] is a function to return if it is available the hash associated
       to the absolute offset.}

      {- [extern] is a function to must return the git object expected.
       Otherwise, we return an {!error} {!Invalid_hash}.}}

      Then, the client must to provide a valid file descriptor of the PACK file
       and can specify how many window(s) the decoder can make (must be upper
        than 1). *)

  val idx : t -> (Hash.t -> (Crc32.t * int64) option)
  (** [idx t] provides the [idx] function noticed by the client when he {!make} [t]. *)

  val cache : t -> (Hash.t -> Object.t option)
  (** [cache t] provides the [cache] function noticed by the client when he {!make} [t]. *)

  val revidx : t -> (int64 -> Hash.t option)
  (** [revidx t] provides the [revidx] function noticed by the client when he {!make} [t]. *)

  val extern : t -> (Hash.t -> (kind * Cstruct.t) option Lwt.t)
  (** [extern t] provides the [extern] function noticed  by the client when he {!make} [t]. *)
  ...
  (** [length ?chunk t hash tmp window] returns the length of the git object. To
      be care, it's not the length needed to get the git object - sometimes the
      length needed is more than the length of the object requested (for this request, see {!needed}).

      [?chunk] is how many byte(s) the client wants to fill to the internal
      decoder {!P.t} when it returns [`Await].

      Then, the client need to specify the [tmp] buffer and the [window] used by
      the internal decoder {!P.t} (see {!P.default}). *)

  val needed : ?chunk:int -> ?cache:(Hash.t -> int option) -> t -> Hash.t -> Cstruct.t -> Inflate.window -> (int, error) result Lwt.t
  (** [needed ?chunk ?cache t hash tmp window] returns the maximum size needed
     to store the object identified by [hash].

     [?chunk] is how many byte(s) the client wants to fill to the internal decoder
     {!P.t} when it returns [`Await].

     [?cache] is used to get directly (without de-serialization) the maximum
     size needed for the requested object (consider than is not the same as
     [hash]).
  ...
  (** [optimized_get' ?chunk t absolute_offset (raw0, raw1, length) tmp window]
     returns the object located to the [absolute_offset] in the PACK file. This
     function does not allocate any buffer. It uses [raw0] and [raw1] to
     undelta-ify the git object requested - so [raw0] and [raw1] must need to be
     well-sized to store all PACK object needed to de-serialize the requested
     object. Then, we returns an {!Object.t} which contains a [Cstruct.sub] in
     the field [Object.raw] physically equal to [raw0] or [raw1].

     The client need to notice the [tmp] and the [window] needed by the internal
     decoder {!P.t}.

     And [?chunk] corresponds to how many byte(s) the client wants to fill to
     the internal decoder {!P.t} when it returns [`Await]. *)
  ...
  (** [optimized_get] has the same purpose than {!optimized_get'} but instead to
     notice the absolute offset of the requested object, the client notify the
     unique hash identifier. Internally, we use the [idx] function needed by
     {!make} to get the absolute offset. Otherwise, we returns an
     [Invalid_hash]. *)

  val get' : ?chunk:int -> t -> int64 -> Cstruct.t -> Inflate.window -> (Cstruct.t * Cstruct.t) -> (Object.t, error) result Lwt.t
  (** [get'] has the same purpose than {!optimized_get'} but it checks if [raw0]
     and [raw1] are well-sized to contain all object needed to undelta-ify the
     requested object and to contain the requested object. *)
  ...
  (** [get_with_allocation ?chunk t hash tmp window] has the same purpose than
     {!optimized_get} but it allocates what it needed to store the requested
     object. *)
