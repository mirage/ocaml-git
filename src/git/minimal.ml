module type S = sig
  ...
  *)
  ...
  type pack
  (** Type of a pack object which contains multiple Git objects. *)

  type index
  (** Type of an indexation value of the {!pack} object. *)

  val batch_write : t -> index:index -> pack:pack -> (unit, error) result Lwt.t
  (** [batch_write state ~index ~pack] writes all objects {i indexed} into
