...
module Window = struct
  ...
  let pp ppf window =
    Fmt.pf ppf "{ @[<hov>raw = #raw;@ off = %Ld;@ len = %d;@] }" window.off
      window.len
end
...
module type H = sig
  ...
  type error = Reserved_opcode of int | Wrong_copy_hunk of int * int * int
  ...
  and res = Wait of t | Error of t * error | Cont of t | Ok of t * hunks

  and hunk = Insert of Cstruct.t | Copy of int * int

  and reference = Offset of int64 | Hash of Hash.t

  and hunks =
    {reference: reference; length: int; source_length: int; target_length: int}
...
module Hunk (Hash : S.HASH) = struct
  ...
  let pp_error ppf = function
    | Reserved_opcode byte -> Fmt.pf ppf "Got reserved op-code %02d" byte
    | Wrong_copy_hunk (off, len, source) ->
        Fmt.pf ppf
          "The copy hunk (off: %d, len: %d) is invalid, it's not inner the \
           source (source: %d)."
          off len source
  ...
  type t =
    ...
    ; read:
        int
        (* XXX(dinosaure): consider than it's not possible to have a hunk
           serialized in bigger than [max_native_int] bytes. In practice, it's
           not common to have a Hunks object serialized in a chunk upper than
           [max_int32]. But this case can happen and, because the architecture
           of this deserialization is non-blocking, we can fix this detail but
    ...
    ; _hunk:
        hunk option
        (* XXX(dinosaure): about memory optimization, this deserializer returns
           each hunk and the user can accumulate or process in the same time
           the git object. so, we allocate a [Cstruct.t] for each [Insert] and
           keep one [Cstruct.t] and used for each [Insert] returned. but
           internally, we accumulate (and copy/allocate) each hunk. in others
           words, we can avoid this and use only the non-blocking memory
           efficient way but I'm lazy to re-implement some top functions like
           [Decoder.get] to construct in the same time the git object
           requested. TODO! *)
    ; _tmp: Cstruct.t
    ; state: state }
  ...
  let pp_state ppf = function
    ...
    | List _ -> Fmt.string ppf "(List #k)"
    | Is_insert (_, off, len) ->
        Fmt.pf ppf "(Is_insert (#raw, %d, %d))" off len
    | Is_copy _ -> Fmt.string ppf "(Is_copy #k)"
    | End -> Fmt.string ppf "End"
    | Exception exn -> Fmt.pf ppf "(Exception %a)" (Fmt.hvbox pp_error) exn

  let pp_hunks ppf hunks =
    Fmt.pf ppf
      "{ @[<hov>reference = %a;@ length = %d;@ source_length = %d;@ \
       target_length = %d@] }"
      (Fmt.hvbox pp_reference) hunks.reference hunks.length hunks.source_length
      hunks.target_length

  let pp ppf t =
    Fmt.pf ppf
      "{ @[<hov>i_off = %d;@ i_pos = %d;@ i_len = %d;@ read = %d;@ length = \
       %d;@ reference = %a;@ source_length = %d;@ target_length = %d;@ state \
       = %a;@] }"
      t.i_off t.i_pos t.i_len t.read t._length (Fmt.hvbox pp_reference)
      t._reference t._source_length t._target_length (Fmt.hvbox pp_state)
      t.state
      ...
      | true ->
          get_byte
            (fun byte src t ->
              ...
              (length [@tailcall]) msb
                (((byte land 0x7F) lsl bit) lor len, bit + 7)
                k src t )
            src t
      | false -> k len src t
  ...
  module KList = struct let get_byte = get_byte ~ctor:(fun k -> List k) end

  let copy opcode src t =
    let rec get_byte flag k src t =
      if not flag then k 0 src t
        ...
        await
          { t with
            state= Is_copy (fun src t -> (get_byte [@tailcall]) flag k src t)
          }
    in
    ( get_byte (opcode land 0x01 <> 0)
    @@ fun o0 ->
    get_byte (opcode land 0x02 <> 0)
    @@ fun o1 ->
    get_byte (opcode land 0x04 <> 0)
    @@ fun o2 ->
    get_byte (opcode land 0x08 <> 0)
    @@ fun o3 ->
    get_byte (opcode land 0x10 <> 0)
    @@ fun l0 ->
    get_byte (opcode land 0x20 <> 0)
    @@ fun l1 ->
    get_byte (opcode land 0x40 <> 0)
    @@ fun l2 _ t ->
    let dst = ref 0 in
    let len = ref 0 in
    ...
    if dst + len > t._source_length then
      error t (Wrong_copy_hunk (dst, len, t._source_length))
      (* Cont { t with state = Stop } (* avoid error *) *)
    else Cont {t with state= Stop; _hunk= Some (Copy (dst, len))} )
      src t

  let stop _ t = Cont t

  let list src t =
    if t.read < t._length then
      KList.get_byte
        (fun opcode _ t ->
          if opcode = 0 then error t (Reserved_opcode opcode)
            ...
            | 0 ->
                Cont
                  { t with
                    state= Is_insert (Cstruct.sub t._tmp 0 opcode, 0, opcode)
                  }
            | _ -> Cont {t with state= Is_copy (copy opcode)} )
        src t
    else ok {t with _hunk= None}
  ...
  let header src t =
    ...
    Cont {t with state= List list; _source_length; _target_length} )
      src t
  ...
  let eval src t =
    ...
    let rec loop t =
      match eval0 t with
      | Cont ({state= Stop; _hunk= Some hunk; _} as t) -> `Hunk (t, hunk)
      | Cont {state= Stop; _} -> assert false
      | Cont
          ( { state=
                Header _ | List _ | Is_insert _ | Is_copy _ | End | Exception _; _
            } as t ) ->
          loop t
      | Wait t -> `Await t
  ...
  let current t =
    match t with
    | {state= Stop; _hunk= Some hunk; _} -> hunk
    | {state= Stop; _hunk= None; _}
     |{ state= Header _ | List _ | Is_insert _ | Is_copy _ | End | Exception _
      ; _hunk= Some _ | None; _ } ->
        raise (Invalid_argument "Hunk.current: bad state")
...
module type P = sig
  ...
  type kind = Commit | Tree | Blob | Tag | Hunk of Hunk.hunks
...
struct
  ...
  let pp_error ppf = function
    | Invalid_byte byte -> Fmt.pf ppf "Got an invalid byte: %02x" byte
    | Reserved_kind byte -> Fmt.pf ppf "Got an reserved byte: %02x" byte
    | Invalid_kind byte ->
    ...
    | Invalid_length (expected, has) ->
        Fmt.pf ppf
          "Unserialize a corrupted object (length mismatch, expect %d, have %d)"
          expected has
    | Hunk_error err ->
        Fmt.pf ppf "Got a Hunk decoder error: %a" Hunk.pp_error err
    | Hunk_input _ -> assert false

  type process = [`All | `One | `Length | `Metadata]
  ...
  and state =
    ...
    | Next of
        ...
        ; length':
            int
            (* XXX(dinosaure): [length] is the value decoded. [length'] is the
               value returned when we inflate the raw. It must to be the same.
               However, we can inflate a huge object (like a [Blob] which is
               not delta-ified).

               The length of the object can be upper than [max_native_int] (but
               can't be upper than [max_int64]). So we need to switch these
  ...
  and kind = Commit | Tree | Blob | Tag | Hunk of Hunk.hunks
  ...
  let pp_hunks_state ppf {offset; length; consumed; z; h; _} =
    Fmt.pf ppf
      "{ @[<hov>offset = %Ld;@ consumed = %d;@ length = %d;@ z = %a;@ h = \
       %a;@] })"
      offset consumed length (Fmt.hvbox Inflate.pp) z (Fmt.hvbox Hunk.pp) h

  let pp_state ppf = function
    | Header _ -> Fmt.string ppf "(Header #k)"
    | Object _ -> Fmt.string ppf "(Object #k)"
    | VariableLength _ -> Fmt.string ppf "(VariableLength #k)"
    | Unzip {offset; consumed; length; kind; z; _} ->
        Fmt.pf ppf
          "(Unzip { @[<hov>offset = %Ld;@ consumed = %d;@ length = %d;@ kind \
           = %a;@ z = %a;@] })"
          offset consumed length pp_kind kind (Fmt.hvbox Inflate.pp) z
    | Hunks hs -> Fmt.pf ppf "(Hunks %a)" (Fmt.hvbox pp_hunks_state) hs
    | StopHunks hs -> Fmt.pf ppf "(StopHunks %a)" (Fmt.hvbox pp_hunks_state) hs
    | Next {offset; consumed; length; length'; _} ->
        Fmt.pf ppf
          "(Next { @[<hov>offset = %Ld;@ consumed = %d;@ length = %d;@ \
           length' = %d;@] })"
          offset consumed length length'
    | Checksum _ -> Fmt.string ppf "(Checksum #k)"
    | End hash -> Fmt.pf ppf "(End %a)" Hash.pp hash
    | Exception err -> Fmt.pf ppf "(Exception %a)" (Fmt.hvbox pp_error) err

  let pp ppf t =
    Fmt.pf ppf
      "{ @[<hov>i_off = %d;@ i_pos = %d;@ i_len = %d;@ version = %ld;@ \
       objects = %ld;@ counter = %ld;@ state = %a;@] }"
      t.i_off t.i_pos t.i_len t.version t.objects t.counter
  ...
  let rec get_byte ~ctor k src t =
    if t.i_len - t.i_pos > 0 then
      let byte = Cstruct.get_uint8 src (t.i_off + t.i_pos) in
      k byte src {t with i_pos= t.i_pos + 1; read= Int64.add t.read 1L}
    else
      await
        {t with state= ctor (fun src t -> (get_byte [@tailcall]) ~ctor k src t)}
  ...
  module KHeader = struct
    let rec check_byte chr k src t =
      ...
      then k src {t with i_pos= t.i_pos + 1; read= Int64.add t.read 1L}
      else if t.i_len - t.i_pos = 0 then
        await
          { t with
        ...
        error
          {t with i_pos= t.i_pos + 1; read= Int64.add t.read 1L}
    ...
    let rec get_u32 k src t =
      if t.i_len - t.i_pos > 3 then
        ...
        k num src {t with i_pos= t.i_pos + 4; read= Int64.add t.read 4L}
      else if t.i_len - t.i_pos > 0 then
        ( get_byte
        @@ fun byte0 ->
        get_byte
        ...
        await
          {t with state= Header (fun src t -> (get_u32 [@tailcall]) k src t)}
  end

  module KVariableLength = struct
    let get_byte = get_byte ~ctor:(fun k -> VariableLength k)
  end

  module KObject = struct
    let get_byte = get_byte ~ctor:(fun k -> Object k)
    let get_hash = get_hash ~ctor:(fun k -> Object k)
  end

  module KChecksum = struct let get_hash = get_hash ~ctor:(fun k -> Checksum k)
  end

  let rec length msb (len, bit) crc k src t =
    match msb with
    | true ->
  ...
  let rec offset msb off crc k src t =
    match msb with
    | true ->
        KVariableLength.get_byte
          (fun byte src t ->
            ...
            (offset [@tailcall]) msb
              (Int64.logor
                 (Int64.shift_left (Int64.add off 1L) 7)
                 (Int64.of_int (byte land 0x7F)))
              crc k src t )
          src t
    | false -> k off crc src t

  let stop_hunks _ t hs = Cont {t with state= StopHunks hs}

  (* XXX(dinosaure): Need an explanation. We must compute firstly the
     evaluation of the [H] before [Inflate] to be sure we wait something (and
     ask only when it's needed [Inflate]).
     ...
     (so, you are free to do what you want with [t.o_z]) - We [Inflate] only
     when it's needed (so, when [H] returns [`Await]) *)
  let hunks src t offset length consumed crc z h =
    ...
    | `Await h -> (
      ...
      | `Await z ->
          ...
          let crc =
            Crc32.digest ~off:(t.i_off + t.i_pos) ~len:(Inflate.used_in z) crc
              src
          in
          await
            { t with
              state=
                Hunks
                  {offset; length; consumed; crc; z= Inflate.refill 0 0 z; h}
          ...
          { t with
            state=
              Unzip
                ...
                ; kind= Commit
                ; z=
                    Inflate.flush 0 (Cstruct.len t.o_z)
                  ...
                  { t with
                    state=
                      Hunks
                        ...
                        ; length= len
                        ; consumed=
                            size_of_variable_length len + size_of_offset offset
                        ; crc
                        ; z=
                            Inflate.flush 0 (Cstruct.len t.o_z)
                            @@ Inflate.refill (t.i_off + t.i_pos)
                                 (t.i_len - t.i_pos)
                            @@ Inflate.default (Inflate.window_reset t.o_w)
  ...
  let checksum src t = KChecksum.get_hash (fun hash _ t -> ok t hash) src t
      ...
      (fun byte src t ->
        ...
        length msb
          ...
          (fun len crc _ t ->
            Cont
              { t with
                state= Object (fun src t -> switch typ offset len crc src t) }
            )
          src t )
      src t
    ...
    | `Await z ->
        ...
        await
          { t with
            state=
              Unzip
                {offset; length; consumed; crc; kind; z= Inflate.refill 0 0 z}
          ; i_pos= t.i_pos + Inflate.used_in z
          ; read= Int64.add t.read (Int64.of_int (Inflate.used_in z)) }
    | `Flush z ->
        flush {t with state= Unzip {offset; length; consumed; crc; kind; z}}
    | `End z ->
        let crc =
          Crc32.digest ~off:(t.i_off + t.i_pos) ~len:(Inflate.used_in z) crc
            src
  ...
  let version src t =
    KHeader.get_u32 (fun version src t -> number src {t with version}) src t
  ...
  let is_end {state; _} =
    match state with
    | End _ -> true
    | Header _ | Object _ | VariableLength _ | Unzip _ | Hunks _
     |StopHunks _ | Next _ | Checksum _ | Exception _ ->
        false

  let refill off len t =
    if t.i_len - t.i_pos = 0 && not (is_end t) then
      match t.state with
      ...
      | Hunks {offset; length; consumed; crc; z; h} ->
          ...
          ; state=
              Hunks
                {offset; length; consumed; crc; z= Inflate.refill off len z; h}
          }
      | Header _ | Object _ | VariableLength _ | End _ | StopHunks _ | Next _
       |Checksum _ | Exception _ ->
          {t with i_off= off; i_len= len; i_pos= 0}
    else if is_end t then {t with i_off= off; i_len= len; i_pos= 0}
      (* XXX(dinosaure): at the end, we don't care if we lost something. *)
    else
      raise
        (Invalid_argument
           (Fmt.strf "Pack.refill: you lost something (pos: %d, len: %d)"
              t.i_pos t.i_len))
  ...
  let flush off len t =
    ...
    | Unzip {offset; length; consumed; crc; kind; z} ->
        { t with
          state=
            Unzip
              {offset; length; consumed; crc; kind; z= Inflate.flush off len z}
        }
    | End _ | Header _ | Object _ | VariableLength _ | Hunks _ | StopHunks _
     |Next _ | Checksum _ | Exception _ ->
        raise (Invalid_argument "Pack.flush: bad state")

  let output t =
    match t.state with
    | Unzip {z; _} -> t.o_z, Inflate.used_out z
    | End _ | Header _ | Object _ | VariableLength _ | Hunks _ | StopHunks _
     |Next _ | Checksum _ | Exception _ ->
  ...
  let next_object t =
    ...
    | Next _ when t.process = `All ->
        if Int32.pred t.counter = 0l then
          {t with state= Checksum checksum; counter= Int32.pred t.counter}
        else {t with state= Object kind; counter= Int32.pred t.counter}
    | Next _ ->
        { t with
          state= End (Hash.of_raw_string (String.make Hash.digest_size '\000'))
        }
    (* XXX(dinosaure): in the local case, the user don't care about the hash of
       the PACK file. *)
    | End _ | Header _ | Object _ | VariableLength _ | Hunks _ | StopHunks _
     |Unzip _ | Checksum _ | Exception _ ->
        raise (Invalid_argument "Pack.next_object: bad state")

  let kind t =
    match t.state with
    | Unzip {kind; _} -> kind
    | StopHunks {h; _} | Hunks {h; _} -> Hunk (Hunk.partial_hunks h)
    | Next {kind; _} -> kind
    | End _ | Header _ | Object _ | VariableLength _ | Checksum _ | Exception _
      ->
  ...
  (* XXX(dinosaure): The consumed value calculated in this deserialization is
     different from what git says (a diff of 1 or 2 bytes) - may be it come
     from a wrong compute of the length of the offset value (see
     {!size_of_offset}). It's not very important but FIXME! *)
  let consumed t =
    match t.state with
  ...
  let continue t =
    match t.state with
    | StopHunks hs -> {t with state= Hunks {hs with h= Hunk.continue hs.h}}
    | End _ | Header _ | Object _ | VariableLength _ | Hunks _ | Next _
     |Unzip _ | Checksum _ | Exception _ ->
  ...
  let eval src t =
    let rec loop t =
      ...
      | Cont ({state= Next _; _} as t) -> `Object t
      | Cont ({state= StopHunks hs; _} as t) -> `Hunk (t, Hunk.current hs.h)
      | Cont
          ( { state=
                ( Header _ | Object _ | VariableLength _ | Unzip _ | Hunks _
                | Checksum _ | End _ | Exception _ ); _ } as t ) ->
          loop t
      | Wait t -> `Await t
      | Flush t -> `Flush t
  ...
  let eval_metadata src t =
    let rec loop t =
      ...
      match eval0 src t with
      | Cont
          ( ({state= Next _; _} | {state= Unzip _; _} | {state= Hunks _; _}) as
          t ) ->
          `Metadata t
      | Cont ({state= StopHunks _; _} as t) -> loop (continue t)
      | Cont t -> loop t
    ...
    val apply_from_hash :
      ...
      -> cache:( int64
               , kind
                 * Cstruct.t
                 * int
                 * [`Patch of metadata | `Base of metadata | `Extern] )
               Cache.t
      -> ?chunk:int
      -> ?htmp:Cstruct.t array
      -> Cstruct.t * Cstruct.t
...
struct
  ...
  let pp_error ppf = function
    ...
    | Invalid_offset off -> Fmt.pf ppf "Invalid offset: %Ld" off
    | Invalid_target (has, expected) ->
        Fmt.pf ppf
          "Bad object re-constructed (length mismatch, expect: %d, have: %d)"
          expected has
    | Unpack_error (_, _, exn) ->
        Fmt.pf ppf "Got an error while decoding PACK: %a" Pack.pp_error exn
    | Mapper_error err ->
        Fmt.pf ppf "Got an error while mmaping: %a" Mapper.pp_error err
  ...
  let make ?(bucket = 10) file idx get =
    ...
    >|= function
    | Error _ as err -> err
    | Ok max -> Ok {file; max; win= Bucket.make bucket; idx; get}

  module Diff = struct
    type insert = S of string | C of Cstruct.t
    type t = Insert of insert | Copy of (int * int)

    let blit_to_cstruct src off0 dst off1 len =
      match src with
  ...
  let map_window t offset_requested =
    let pos = Int64.(offset_requested / 1024L * 1024L) in
    (* padding *)
    Mapper.map t.file ~pos (1024 * 1024)
    >>= function
    | Ok map ->
        Lwt.return (Ok {Window.raw= map; off= pos; len= Cstruct.len map})
  ...
  let find_window t offset_requested =
    ...
    | Some window ->
        Log.debug (fun l ->
            l "Reusing a window already loaded: [%Ld:%d]" window.Window.off
              window.Window.len ) ;
        let relative_offset =
    ...
    | None -> (
        Log.debug (fun l ->
            l "Loading a new window for the %Ld offset requested."
              offset_requested ) ;
  ...
  module Patch = struct
    ...
    let get_from_absolute_offset ~ztmp ~zwin ?(chunk = 0x8000) ?htmp t
        absolute_offset =
      ...
      | Ok (window, relative_offset) ->
          ...
          let rec go window consumed_in_window writed_in_htmp hunks result
              state =
            match Pack.eval window.Window.raw state with
            | `Await state -> (
                let rest_in_window =
                  min (window.Window.len - consumed_in_window) chunk
                in
                if rest_in_window > 0 then
                  go window
                  ...
                  find_window t
                    Int64.(window.Window.off + of_int consumed_in_window)
                  >>= function
                  | Error err -> Lwt.return_error (Mapper_error err)
                  | Ok (window, relative_offset) ->
                      go window relative_offset writed_in_htmp hunks result
            ...
            | `Hunk (state, Hunk.Insert raw) ->
                ...
                let insert =
                  ...
                  | None -> Diff.S (Cstruct.to_string raw)
                in
                go window consumed_in_window (writed_in_htmp + len)
                  (Diff.Insert insert :: hunks)
                  result (Pack.continue state)
            | `Hunk (state, Hunk.Copy (off, len)) ->
                go window consumed_in_window writed_in_htmp
            ...
            | `Object state ->
                let descr =
                  ...
                  | _ -> invalid_arg "Object is not a patch"
                in
                let patch =
                  ...
                  ; descr }
                in
                go window consumed_in_window writed_in_htmp hunks (Some patch)
    ...
    let apply_from_absolute_offset ~ztmp ~zwin ?(chunk = 0x8000) (kind, source)
        rtmp t absolute_offset =
      find_window t absolute_offset
      >>= function
      | Error err -> Lwt.return_error (Mapper_error err)
      | Ok (window, relative_offset) ->
          let state = Pack.from_window window relative_offset ztmp zwin in
          let rec go window consumed_in_window writed_in_rtmp writed_in_htmp
              ctx result state =
            match Pack.eval window.Window.raw state with
            | `Await state -> (
                let rest_in_window =
                ...
                if rest_in_window > 0 then
                  ...
                  | Error err -> Lwt.return_error (Mapper_error err)
                  | Ok (window, relative_offset) ->
                      go window relative_offset writed_in_rtmp writed_in_htmp
                        ctx result (Pack.refill 0 0 state) )
            | `Hunk (state, Hunk.Insert raw) ->
                let len = Cstruct.len raw in
                let ctx =
                  match ctx with
                ...
                go window consumed_in_window (writed_in_rtmp + len)
                  (writed_in_htmp + len) ctx result (Pack.continue state)
            | `Hunk (state, Hunk.Copy (off, len)) ->
                let ctx =
                  match ctx with
                  | Some ctx ->
                      let ctx =
                        Hash.feed_cstruct ctx (Cstruct.sub source off len)
                      in
                      Some ctx
                  | None ->
                      let len' =
                ...
                go window consumed_in_window (writed_in_rtmp + len)
                  writed_in_htmp ctx result (Pack.continue state)
            | `Flush _ -> invalid_arg "Object is not a patch"
            | `Object state ->
                let descr =
                  match Pack.kind state with
                ...
                let patch =
                  ...
                  ; descr }
                in
                go window consumed_in_window writed_in_rtmp writed_in_htmp None
  ...
  module Base = struct
    ...
    let get_from_absolute_offset ~ztmp ~zwin ?(chunk = 0x8000) rtmp t
        absolute_offset =
      find_window t absolute_offset
      >>= function
      | Error err -> Lwt.return_error (Mapper_error err)
          ...
          let rec go window consumed_in_window writed_in_rtmp ctx result state
            ...
            | `Await state -> (
                ...
                if rest_in_window > 0 then
                  ...
                  | Error err -> Lwt.return_error (Mapper_error err)
                  | Ok (window, relative_offset) ->
                      go window relative_offset writed_in_rtmp ctx result
                        (Pack.refill 0 0 state) )
            ...
            | `Flush state ->
                ...
                let ctx =
                  ...
                  | Some ctx ->
                      let ctx =
                        Hash.feed_cstruct ctx (Cstruct.sub buf 0 len)
                      in
                      Some ctx
                  | None ->
                      let ctx = Hash.init () in
                ...
                go window consumed_in_window (writed_in_rtmp + len) ctx result
            ...
            | `Object state ->
                ...
                in
                go window consumed_in_window writed_in_rtmp None (Some base)
                  (Pack.next_object state)
            | `Error (state, exn) ->
                Lwt.return_error (Unpack_error (state, window, exn))
            | `End _ -> (
  ...
  module Object = struct
    ...
    let get_from_absolute_offset ~ztmp ~zwin ?(chunk = 0x8000) ?htmp rtmp t
        absolute_offset =
      find_window t absolute_offset
      >>= function
      | Error err -> Lwt.return_error (Mapper_error err)
          ...
          let rec go window consumed_in_window writed_in_rtmp writed_in_htmp
            ...
            | `Await state -> (
                ...
                if rest_in_window > 0 then
                  ...
                  | Ok (window, relative_offset) ->
                      go window relative_offset writed_in_rtmp writed_in_htmp
                        ctx hunks result (Pack.refill 0 0 state) )
            | `Hunk (state, Hunk.Insert raw) ->
                let len = Cstruct.len raw in
                let insert =
                  match htmp with
                  ...
                  | None -> Diff.S (Cstruct.to_string raw)
                in
                go window consumed_in_window writed_in_rtmp
                  (writed_in_htmp + len) ctx
                  (Diff.Insert insert :: hunks)
                  result (Pack.continue state)
            | `Hunk (state, Hunk.Copy (off, len)) ->
                go window consumed_in_window writed_in_rtmp writed_in_htmp ctx
                  (Diff.Copy (off, len) :: hunks)
                  result (Pack.continue state)
            | `Flush state ->
                let buf, len = Pack.output state in
                ...
                Cstruct.blit buf 0 rtmp writed_in_rtmp len ;
                go window consumed_in_window (writed_in_rtmp + len)
                  writed_in_htmp ctx hunks result
                  (Pack.flush 0 (Cstruct.len buf) state)
            | `Object state ->
                let obj =
                  match Pack.kind state with
                  ...
                  | Pack.Hunk descr ->
                      Patch
                        { Patch.length= Pack.length state
                        ; consumed= Pack.consumed state
                        ; offset= Pack.offset state
                        ; crc= Pack.crc state
                        ; hunks= List.rev hunks
                ...
                in
                go window consumed_in_window writed_in_rtmp writed_in_htmp None
                  [] (Some obj) (Pack.next_object state)
            | `Error (state, exn) ->
                Lwt.return_error (Unpack_error (state, window, exn))
            | `End _ -> (
              match result with
  ...
  let memoize promote find process =
    let memoized x =
      match find x with
      | Some v -> Lwt.return v
      | None -> process x >>= fun y -> promote x y ; Lwt.return y
    in
    memoized
  ...
  let memoize_rec ~promote ~find process_no_rec =
    ...
    let process_rec =
      memoize promote find (fun x -> process_no_rec !process x)
    in
    process := process_rec ;
    process_rec

  module Ascendant = struct
    type t =
      | External of {hash: Hash.t; kind: kind; raw: Cstruct.t}
      | Root of Base.t
      | Node of {patch: Patch.t; source: t}
    ...
    let metadata_of_patch : Patch.t -> metadata =
     fun patch ->
    ...
    let needed_cache weight =
      let module Int64 = struct include Int64

                                let hash n = Hashtbl.hash n
      end in
      let module Int = struct type t = int

                              let weight _ = 1 end in
    ...
    let get_cache weight =
      ...
      let module Path = struct type nonrec t = t
    ...
    let apply_cache weight =
      ...
      let module Object = struct
        type t = kind * Cstruct.t * int * s

        let weight (_, raw, _, _) = Cstruct.len raw
      end in
      Cache.lru weight (module Int64) (module Object)

    let get_from_absolute_offset ~ztmp ~zwin ~cache ?(chunk = 0x8000) ?htmp
        rtmp t absolute_offset =
      let get_free_htmp htmp depth =
        match htmp with
        | Some htmp ->
            if depth < Array.length htmp then Some htmp.(depth) else None
        | None -> None
      in
      let go mu (absolute_offset, depth) =
        Object.get_from_absolute_offset ~ztmp ~zwin ~chunk
          ?htmp:(get_free_htmp htmp depth) rtmp t absolute_offset
        >>= function
        | Error err -> Lwt.return_error err
        | Ok (Object.Root base) -> Lwt.return_ok (Root base)
        | Ok
            (Object.Patch
              ( {Patch.descr= {Hunk.reference= Hunk.Offset rel_off; _}; _} as
              patch )) -> (
            mu (Int64.(sub absolute_offset rel_off), succ depth)
            >>= function
      ...
      -> (
          ...
          | None -> (
              ...
              | Some (kind, raw) ->
                  Lwt.return_ok
                    (Node {patch; source= External {hash; kind; raw}})
      ...
      in
      let find (abs_off, _) =
        match cache.Cache.find abs_off with
    ...
    let get_from_hash ~ztmp ~zwin ~cache ?(chunk = 0x8000) ?htmp rtmp t hash =
      match t.idx hash with
      | Some (_, abs_off) ->
          get_from_absolute_offset ~ztmp ~zwin ~cache ~chunk ?htmp rtmp t
            abs_off
      | None -> Lwt.return_error (Invalid_hash hash)
    ...
    let reconstruct rtmp =
      let get_free_rtmp (rtmp0, rtmp1) = function
        | true -> rtmp0
        | false -> rtmp1
      in
      let apply source patch target =
        let target_length =
          List.fold_left
            (fun target_off -> function
              | Diff.Copy (off, len) ->
                  Cstruct.blit source off target target_off len ;
                  target_off + len
              | Diff.Insert insert ->
                  Diff.blit_to_cstruct insert 0 target target_off
                    (Diff.len insert) ;
                  target_off + Diff.len insert )
            0 patch.Patch.hunks
        in
        if target_length <> patch.Patch.descr.Hunk.target_length then
          invalid_arg "apply" ;
        Cstruct.sub target 0 patch.Patch.descr.Hunk.target_length
      in
      let rec go swap depth = function
        | External {kind; raw; _} -> kind, raw, depth, `Extern
        | Root ({Base.kind; Base.raw; _} as base) ->
            kind, raw, depth, `Base (metadata_of_base base)
        | Node {patch; source} ->
            let kind, raw, deepen, _ = go (not swap) (succ depth) source in
            ( kind
            , apply raw patch (get_free_rtmp rtmp swap)
            , deepen
            , `Patch (metadata_of_patch patch) )
      in
      go true 0

    let apply_from_absolute_offset ~ztmp ~zwin ~cache ?(chunk = 0x8000) ?htmp
        rtmp t absolute_offset =
      let get_free_rtmp (rtmp0, rtmp1) = function
        | true -> rtmp0
        | false -> rtmp1
      ...
      let go mu (absolute_offset, switch, depth) =
        Object.get_from_absolute_offset ~ztmp ~zwin ~chunk
          ?htmp:(get_free_htmp htmp depth)
          (get_free_rtmp rtmp switch)
          t absolute_offset
        >>= function
        | Error err -> Lwt.return_error err
        | Ok (Object.Root base) ->
            Lwt.return_ok
              ...
              , base.Base.raw
              , depth
              , `Base (metadata_of_base base) )
        | Ok
            (Object.Patch
              ( {Patch.descr= {Hunk.reference= Hunk.Offset rel_off; _}; _} as
              patch )) -> (
            mu (Int64.(sub absolute_offset rel_off), not switch, succ depth)
            >>= function
            | Ok (kind, source, deepen, _) ->
                Lwt.return_ok
                  ( kind
                  , apply source patch (get_free_rtmp rtmp switch)
                  , deepen
                  , `Patch (metadata_of_patch patch) )
            | Error err -> Lwt.return_error err )
        | Ok
      ...
      -> (
          ...
          | None -> (
              ...
              | Some (kind, raw) ->
                  Lwt.return_ok
                    ( kind
                    , apply raw patch (get_free_rtmp rtmp switch)
                    , depth
                    , `Patch (metadata_of_patch patch) )
              | None -> Lwt.return_error (Invalid_hash hash) ) )
      in
      let find (abs_off, _, _) =
        match cache.Cache.find abs_off with
        | Some (kind, raw, depth, metadata) ->
            Some (Ok (kind, raw, depth, metadata))
        | None -> None
      in
    ...
    let apply_from_hash ~ztmp ~zwin ~cache ?(chunk = 0x8000) ?htmp rtmp t hash
      ...
      match t.idx hash with
      | Some (_, abs_off) ->
          apply_from_absolute_offset ~ztmp ~zwin ~cache ~chunk ?htmp rtmp t
            abs_off
      | None -> Lwt.return_error (Invalid_hash hash)

    let apply ~ztmp ~zwin ~cache ?(chunk = 0x8000) ?htmp rtmp t = function
      | `Hash hash ->
          apply_from_hash ~ztmp ~zwin ~cache ~chunk ?htmp rtmp t hash
      | `Offset absolute_offset ->
          apply_from_absolute_offset ~ztmp ~zwin ~cache ~chunk ?htmp rtmp t
            absolute_offset

    let length_from_absolute_offset ~ztmp ~zwin ?(chunk = 0x8000) t
        absolute_offset =
      find_window t absolute_offset
      >>= function
      | Error err -> Lwt.return_error (Mapper_error err)
      | Ok (window, relative_offset) ->
          ...
          let rec go window consumed_in_window state =
            ...
            | `Length state -> (
              match Pack.kind state with
              | Pack.Hunk ({Hunk.target_length; source_length; _} as descr) ->
                  Lwt.return_ok (`Patch (descr, target_length, source_length))
              | _ -> Lwt.return_ok (`Base (Pack.length state)) )
          in
          go window relative_offset state

    let needed_from_absolute_offset ~ztmp ~zwin ~cache ?(chunk = 0x8000) t
        absolute_offset =
      let go mu (needed, absolute_offset) =
        length_from_absolute_offset ~ztmp ~zwin ~chunk t absolute_offset
        >>= function
        | Error err -> Lwt.return_error err
        | Ok
            (`Patch
              ( {Hunk.reference= Offset rel_off; _}
              , target_length
              , source_length )) ->
            mu
              ( max (max target_length source_length) needed
              , Int64.(sub absolute_offset rel_off) )
    ...
    let needed_from_hash ~ztmp ~zwin ~cache ?(chunk = 0x8000) t hash =
      match t.idx hash with
      | Some (_, abs_off) ->
          needed_from_absolute_offset ~ztmp ~zwin ~cache ~chunk t abs_off
      | None -> Lwt.return_error (Invalid_hash hash)
    ...
    let length_from_hash ~ztmp ~zwin ?(chunk = 0x8000) t hash =
      match t.idx hash with
      | Some (_, abs_off) ->
          length_from_absolute_offset ~ztmp ~zwin ~chunk t abs_off
      | None -> Lwt.return_error (Invalid_hash hash)
  ...
  module Descendant = struct
    ...
    let get_from_absolute_offset ~ztmp ~zwin
        ~cache:(cache_needed, cache_object) ?(chunk = 0x8000) ~children rtmp t
        absolute_offset =
      Base.get_from_absolute_offset ~ztmp ~zwin ~chunk rtmp t absolute_offset
      >>= function
      | Error err -> Lwt.return_error err
      | Ok base -> (
          let load_base absolute_offset =
            ...
            | None -> (
                Ascendant.needed_from_absolute_offset ~ztmp ~zwin
                  ~cache:cache_needed ~chunk t absolute_offset
                >>= function
                | Error err -> Lwt.return_error err
                | Ok needed -> (
                    let rtmp = Cstruct.create (needed * 2) in
                    let rtmp =
                      Cstruct.sub rtmp 0 needed, Cstruct.sub rtmp needed needed
                    in
                    Ascendant.apply_from_absolute_offset ~ztmp ~zwin
                      ~cache:cache_object ~chunk rtmp t absolute_offset
                    >>= function
                    | Ok (kind, raw, _, _) -> Lwt.return_ok (kind, raw)
                    | Error err -> Lwt.return_error err ) )
          in
          let load (parent_absolute_offset, absolute_offset) =
            load_base parent_absolute_offset
            >>= function
            | Error err -> Lwt.return_error err
            | Ok (parent_kind, parent_raw) -> (
                Ascendant.length_from_absolute_offset ~ztmp ~zwin ~chunk t
                  absolute_offset
                >>= function
                | Error err -> Lwt.return_error err
                | Ok length -> (
                    let target = Cstruct.create length in
                    Patch.apply_from_absolute_offset ~ztmp ~zwin ~chunk
                      (parent_kind, parent_raw) target t absolute_offset
                    >>= function
                    | Error err -> Lwt.return_error err
                    | Ok s ->
                        let metadata =
                          ...
                          ; offset= s.Patch.offset }
                        in
                        cache_object.Cache.promote absolute_offset
                          (s.Patch.kind, s.Patch.raw, 0, `Patch metadata) ;
                        Lwt.return_ok s ) )
          in
          let rec go acc = function
            ...
            | x :: r -> (
                ...
                | Ok patch -> (
                    let children' =
                      children (patch.Patch.offset, patch.Patch.hash)
                      |> List.map (fun abs_off -> patch.Patch.offset, abs_off)
                    in
                    go [] children'
                    >>= function
                    | Error err -> Lwt.return_error err
                    ...
                    | Ok children' ->
                        go
                          ( Node {patch= of_patch patch; children= children'}
                          :: acc )
                          r ) )
          in
          go []
            ( children (base.Base.offset, base.Base.hash)
            |> List.map (fun abs_off -> base.Base.offset, abs_off) )
          >>= function
          | Ok children -> Lwt.return_ok (Root {base= of_base base; children})
          | Error err -> Lwt.return_error err )
  end
...
module Stream (Hash : S.HASH) (Inflate : S.INFLATE) = struct
