...
let make :
 ...
 fun mp ~uid_ln ~uid_rw ~uid_wr ->
  ...
  if i <> 0xff744f63l
  then Fmt.invalid_arg "Invalid IDX file (header: %lx <> %lx)" i 0xff744f63l ;
...
let bsearch idx hash =
  ...
  let rec go sub_off sub_len =
    ...
    (* XXX(dinosaure): prevent a wrong comparison with something outside the
       hashes table. *)
    if sub_off + len = hashes_offset + (idx.uid_ln * idx.n)
    then raise_notrace Not_found ;
    let cmp = compare_bigstring idx { off = sub_off + len; len } hash in

    if cmp == 0
    then { off = sub_off + len; len }
    else if sub_len <= idx.uid_ln
    then raise_notrace Not_found
    else if cmp > 0
...
let isearch idx hash =
  ...
  let rec go low high =
    ...
    then
      ...
      let interpolation =
        floor ((highf -. lowf) *. (hashf -. lef) /. (hef -. lef)) in
...
module N (Uid : UID) : sig
  ...
  let encoder dst ~pack index =
    ...
    let o, o_pos, o_max =
      ...
      | `Buffer _ | `Channel _ ->
          (Bigstringaf.create io_buffer_size, 0, io_buffer_size - 1) in
    {
      dst;
      o;
      o_off = 0;
...
struct
  ...
  let enlarge fd more =
    ...
    then
      if old_capacity + more <= Sys.max_string_length
