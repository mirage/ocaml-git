...
(** [weight_of_offset sched ~map t ~weight offset] returns the [weight] of the
    ...
    PACKv2, an [OBJ_REF_DELTA] {i usually} points to an external object (see {i
    thin}-pack). *)
...
(** {3 Path of object.}
    ...
    A {!path} is the {i delta-chain} of the object. It assumes that a {i
    delta-chain} can not be larger than [60] (see Git assumptions). From it, the
    way to construct an object is well-know and the step to discover if an
