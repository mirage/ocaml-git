...
module Make
  ...
  (** A record to manipulate a {i file-system}.
      ...
      manipulated. [append] is like {!Unix.write}. It appends the given {i
      payload} into the opened [fd]. It shifts the offset of the opened file by
      the length of the given {i payload}. [map] is like {!Unix.map_file}. It
  ...
  (** [verify ~digest filename fs stream] does the first pass to analyze a PACK
      ...
      If the list is empty, the given stream (saved into [filename]) is a {i
      canonic} PACK file. It does not require any external objects to extract
      any of its objects.
