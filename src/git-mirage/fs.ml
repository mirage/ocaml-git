...
let src =
  Logs.Src.create "git-mirage.fs" ~doc:"logs mirage file-system's event"
...
let make locks path =
  try H.find locks path with Not_found ->
    let m = Lwt_mutex.create () in
    H.add locks path m ; m
...
(* mirage-fs-unix paths are separated by '/' and should not end by a separator.
   XXX(samoht): there is probably something to revisit in mirage-fs-unix
   here... *)
...
module Make (FS : Mirage_fs_lwt.S) = struct
  type t = {fs: FS.t; locks: Lwt_mutex.t H.t; current: Fpath.t}
  ...
  let pp_error ppf = function
    | `Destroy (f, e) ->
        Fmt.pf ppf "Error while destroying %a: %s" Fpath.pp f e
  ...
  let is_file t path =
    ...
    Lwt.try_bind
      ...
      (* FIXME: it seems that mirage-fs-unix can also raise Sys_error, see
         https://github.com/mirage/mirage-fs-unix/issues/37 *)
        (function
  ...
  module Dir = struct
    ...
    let exists t path =
      ...
      is_dir t path
      >|= function
      | Ok _ as v -> v | Error (`Exn err) -> err_stat path "%a" Fmt.exn err

    let create t d =
      let path = fpath_to_string d in
      Log.debug (fun l -> l "Dir.create %s" path) ;
      FS.mkdir t.fs path
      >|= function Ok () -> Ok true | Error e -> err_fs_write e

    (* Note: this breaks atomocity of move, but it's used only during `reset`
       operations, so that should be fine. *)
    let delete t d =
      let path = fpath_to_string d in
      Log.debug (fun l -> l "Dir.delete %s" path) ;
      FS.destroy t.fs path
      >|= function Ok () -> Ok () | Error e -> err_fs_write e

    (* Note: this breaks atomicity of move: if a concurrent move is in progress
       [contents] can contain both the source and the destination. Hopefully we
       only move files from a temp directory into its final destination so that
       should be fine.

       XXX(samoht): maybe the primivite operations should be restricted
       somehow? *)
  ...
  module File = struct
    ...
    (* Note: all the operations (including reads) need to be locked because of
       the lack of atomic moves in MirageOS's FS signature. This should
       probably be fixed in mirage-fs directly. *)

    type 'a fd =
      {t: t; fd: string; path: Fpath.t; mutable seek: int}
      constraint 'a = [< `Read | `Write]

    let exists t path =
      Log.debug (fun l -> l "File.exists %s" @@ fpath_to_string path) ;
      with_lock t.locks path
      @@ fun () ->
      is_file t path
      >|= function
      | Ok _ as v -> v | Error (`Exn err) -> err_stat path "%a" Fmt.exn err
    ...
    let open' t ~create path =
      ...
      | Error e -> (
          if not create then Lwt.return (err_fs_read e)
            ...
            FS.create t.fs (fpath_to_string path)
            >|= function Ok () -> Ok fd | Error e -> err_fs_write e )
    ...
    let open_r t path =
      ...
      open' t ~create:false path
      >|?= fun fd -> ({t; path; fd; seek= 0} :> [`Read] fd)

    let open_w t path =
      Log.debug (fun l -> l "File.open_w %s" @@ fpath_to_string path) ;
      open' t ~create:true path
      >|?= fun fd -> ({t; path; fd; seek= 0} :> [`Write] fd)

    let write raw ?(off = 0) ?(len = Cstruct.len raw) fd =
      Log.debug (fun l -> l "File.write %s" fd.fd) ;
      with_lock fd.t.locks fd.path
      @@ fun () ->
      FS.write fd.t.fs fd.fd fd.seek (Cstruct.sub raw off len)
      >|= function
      | Ok () ->
          fd.seek <- fd.seek + len ;
          Ok len
    ...
    let read dst ?(off = 0) ?(len = Cstruct.len dst) fd =
      ...
      with_lock fd.t.locks fd.path
      @@ fun () ->
      FS.read fd.t.fs fd.fd fd.seek len
      >|= function
    ...
    let delete t path =
      ...
      with_lock t.locks path
      @@ fun () ->
      FS.destroy t.fs (fpath_to_string path)
      >|= function Ok _ as v -> v | Error e -> err_fs_write e

    (* [join_results l] is similar to {Lwt.join}: it waits for all elements of
       [l] to be resolved. The result is either [Ok ()] if all the promises are
       resolving to [Ok ()] or [Error e] if there is at least one element
       resolving to [Error e]. *)
    let join_results l =
      let result = ref (Ok ()) in
      let run f = f >|= function Ok () -> () | e -> result := e in
      Lwt.join (List.map run l) >|= fun () -> !result
    ...
    let move t patha pathb =
      ...
      let path1, path2 =
        (* we sort the order in which we lock to avoid dead-locks with 2
           concurrent move a->b and b<-a *)
        if Fpath.compare patha pathb <= 0 then patha, pathb else pathb, patha
      in
      with_lock t.locks path1
      @@ fun () ->
      with_lock t.locks path2
      @@ fun () ->
      open' t patha ~create:false
      >>?= fun fda ->
      open' t pathb ~create:true
      >>?= fun fdb ->
      FS.size t.fs fda
      >>!= fs_read
      >>?= fun size ->
      ...
      let rec write pos () =
        ...
        | None -> Lwt.return (Ok ())
      in
      join_results [read 0 size >>!= fs_read; write 0 () >>!= fs_write]
  end
  ...
  module Mapper = struct
    ...
    let openfile t path =
      ...
      FS.stat t.fs (fpath_to_string path)
      >|= function
    ...
    let map {t; path; _} ?(pos = 0L) len =
      ...
      with_lock t.locks path
      @@ fun () ->
      FS.read t.fs (fpath_to_string path) (Int64.to_int pos) len
      >|= function
      | Ok cs -> Ok (Cstruct.concat cs) | Error e -> Error (`FS_read e)
