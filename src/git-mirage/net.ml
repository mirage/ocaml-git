...
module Make (C: CONDUIT) = struct
  ...
  let read { ic; _ } raw off len =
    ...
    | Ok (`Data cs) ->
      ...
      Lwt.return len'
    | Error err ->
      Log.err (fun l -> l ~header:"read" "Catch error when we read: %a."
                  Channel.pp_error err);
      Lwt.return 0

  let socket uri =
    let open Lwt.Infix in

    Resolver_lwt.resolve_uri ~uri C.resolver >>= fun endp ->
    Conduit_mirage.client endp >>= fun client ->
    Conduit_mirage.connect C.context client >>= fun flow ->
