...
let endpoint ?(conduit=Conduit_mirage.empty) ?resolver ?headers uri =
  let resolver = match resolver with
    | None   -> Resolver_lwt.init ()
    | Some r -> r
  in
...
module Sync (G: Git.S) = struct
  ...
  module Client = struct
    ...
    let call ?headers ?body meth (e:endpoint) =
      ...
      Log.debug (fun l -> l ~header:"call" "Send a request to %a." Uri.pp_hum (uri e)) ;
      Cohttp_mirage.Client.call ~ctx:(ctx e) ?headers ?body ~chunked:false meth (uri e)
      >>= fun ((resp, _) as v) ->
      if
      ...
      then (
        ...
        Log.debug (fun l ->
            l ~header:"call" "Redirection from %a to %a." Uri.pp_hum (uri e)
              Uri.pp_hum uri' ) ;
        Cohttp_mirage.Client.call ~ctx:(ctx e)
          ?headers ~body:(`Stream body_if_redirection)
