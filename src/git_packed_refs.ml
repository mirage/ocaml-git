...
let to_line ppf = function
  ...
  | `Entry (s,r) ->
    Format.fprintf ppf "%s %s" (Git_hash.to_hex s) (Git_reference.to_raw r)
...
let find t r =
  let rec aux = function
    ...
    | `Entry (x, y) :: t -> if Git_reference.equal y r then Some x else aux t
  in
  aux t

module Set = Set.Make(Git_reference)
