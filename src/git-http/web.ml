...
module type S = sig
  ...
  module HTTP: sig
    type headers
    (** The type for header maps.  A header map represents a list of
        HTTP headers. It maps header field names to their value. *)

    type path = string list
    (** The type for HTTP [absolute_path]s represented by its
        non-empty list of URI [segment]s. Note that URI segments can
        be empty; in particular the absolute path ["/"] is the list
        with a single empty segment and is thus represented by the
        list [[""]]. *)

    type version = int * int
    (** The type for representing [HTTP-Version] fields. Both integers
        must be in the interval [\[0;9\]]. *)
    ...
    (** Headers.  FIXME(dbuenzli): The semantic of this module
        considers than a field can contain multiple values. However,
        it's should be the wrong way to abstract values of headers -
        and let the user to consider than specific field can have
        multiple values. *)
    module Headers: sig
      ...
      (** [find n hs] is the value of header [n] in [hs] (if
          defined). If [n] is multi-valued and defined, we return
          [String.concat "," vs] where [vs] are values related to [n]
          in [hs]. *)
  ...
  module Request:  sig
    ...
    (** [headers r] is [r]'s information. Initially empty it can be
        used be services and layers to store and share data. *)
    ...
    val v: ?version:HTTP.version -> HTTP.meth -> path:HTTP.path ->
      ?query:(string * string list) list -> HTTP.headers -> body -> req
