...
module Make (S : Git.S) = struct
  ...
  let references =
    ...
    ; "refs/remotes/upstream/master"; "refs/heads/smart-protocol"
    ; "refs/heads/server"; "refs/heads/smart-tests"
    ; "refs/remotes/origin/smart-protocol"; "refs/remotes/origin/smart-tests"
    ; "refs/remotes/origin/server"; "refs/remotes/upstream/server"
  ...
  module Data = struct
    ...
    let git_proto_request1, git_proto_request_result1 =
      ( { Smart.Common.pathname= path1
        ; host= host1
        ; request_command= `Upload_archive }
      , `Equal )
    ...
    let update_request0, update_request_result0 =
      let commands0 =
        let first =
          Smart.Common.Update
            (generate_hash (), generate_hash (), List.nth references 0)
        in
        let rest = [Smart.Common.Create (zero, List.nth references 1)] in
        `Raw (first, rest)
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
        ; capabilities
    ...
    let update_request1, update_request_result1 =
      let commands1 =
        let first =
          Smart.Common.Update
            (generate_hash (), generate_hash (), List.nth references 1)
        in
        let rest = [Smart.Common.Delete (zero, List.nth references 2)] in
        `Raw (first, rest)
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
        ; capabilities
    ...
    let update_request3, update_request_result3 =
      let commands3 =
        let first =
          Smart.Common.Update
            (generate_hash (), generate_hash (), List.nth references 3)
        in
        `Raw (first, [])
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
    ...
    let update_request4, update_request_result4 =
      let commands4 =
        let first =
          Smart.Common.Create (generate_hash (), List.nth references 4)
        in
        `Raw (first, [])
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
    ...
    let update_request5, update_request_result5 =
      let commands5 =
        let first =
          Smart.Common.Delete (generate_hash (), List.nth references 5)
        in
        `Raw (first, [])
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
    ...
    let update_request6, update_request_result6 =
      let commands6 =
        let first =
          Smart.Common.Update
            (generate_hash (), generate_hash (), List.nth references 6)
        in
        let rest =
          [ Smart.Common.Create (generate_hash (), List.nth references 7)
          ; Smart.Common.Delete (generate_hash (), List.nth references 8) ]
        in
        `Raw (first, rest)
      in
      ( { Smart.Common.shallow= generate_hashes (Random.int 10)
   ...
   fun ~name v trans to_action expect test ->
    ...
    , fun () ->
        ...
        | Ok v', `Error ->
            ...
            if T.equal v v' then
              Alcotest.failf "Expected an errori (equality) with %s: %a" name
                T.pp v
        | Error _, `Error -> ()
        | Error (err, _, committed), `Equal ->
            Fmt.(pf stderr)
              "Input (committed: %02x): %a.\n" committed
              (Fmt.hvbox Encore.Lole.pp_string)
              s ;
            Alcotest.failf "Got an error from the decoder: %a"
              Smart.Decoder.pp_error err )
  ...
  type _ t =
    ...
    | Acks :
        ( Smart.Common.acks
        * S.Hash.t list
        * Smart.Decoder.ack_mode
        * [`Error | `Equal] )
        list
        -> Smart.Common.acks t
    | Negociation_result :
        (Smart.Common.negociation_result * [`Error | `Equal]) list
        -> Smart.Common.negociation_result t
    | Git_proto_request :
        (Smart.Common.git_proto_request * [`Error | `Equal]) list
        -> Smart.Common.git_proto_request t
    | Update_request :
        (Smart.Common.update_request * [`Error | `Equal]) list
        -> Smart.Common.update_request t
  ...
  let testable_of : type v. v t -> (module Alcotest.TESTABLE with type t = v) =
    function
    | Advertised_refs _ ->
        let module T = struct
          type t = v
  ...
  let from_t : type v. v t -> unit Alcotest.test_case list =
   fun t ->
    ...
    | Shallow_update l ->
        List.mapi
          (fun idx (v, e) ->
            make_test ~name:(name idx) v Smart.Decoder.ShallowUpdate
              (fun v -> `Shallow_update v)
              e testable )
          l
    | Acks l ->
        List.mapi
          (fun idx (v, hashes, ack_mode, e) ->
            make_test ~name:(name idx) v
    ...
    | Negociation_result l ->
        List.mapi
          (fun idx (v, e) ->
            make_test ~name:(name idx) v Smart.Decoder.NegociationResult
    ...
    | Git_proto_request l ->
        List.mapi
          (fun idx (v, e) ->
            make_test ~name:(name idx) v Smart.Decoder.Git_proto_request
    ...
    | Update_request l ->
        List.mapi
          (fun idx (v, e) ->
            make_test ~name:(name idx) v Smart.Decoder.Update_request
