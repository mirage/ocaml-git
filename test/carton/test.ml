...
let loads =
  ...
  let map payload ~pos len =
    ...
    if pos > Int64.of_int (Bigstringaf.length payload)
    then failf "mmap: index out of bounds" ;
...
let empty_index, uid_empty_index =
  let res =
    ...
    ( Bos.OS.Dir.with_tmp "git-%s" @@ fun path ->
      ...
      let cmd =
        Bos.Cmd.(
          v "git" % "index-pack" % "--stdin" % "-o" % Fpath.to_string dst) in
...
let verify_bomb_pack () =
  ...
  let rec go decoder =
    ...
    | `Malformed err -> Alcotest.fail err
    | `End uid -> Alcotest.(check sha1) "hash" uid hash_expected in
...
let pack_bomb_pack () =
  ...
  let load uid =
    ...
    | exception Not_found -> Alcotest.failf "Invalid UID %a" Uid.pp uid in
  let entries =
    Array.map
      (fun s ->
        let uid = Verify.uid_of_status s in
        ...
        let length =
          ...
          Carton.Dec.weight_of_offset unix ~map pack ~weight:Carton.Dec.null
            cursor
          >>= fun weight -> return (weight :> int) in
  ...
  let targets =
    D.delta ~threads:[ load ] ~weight:10 ~uid_ln:Uid.length entries in
...
let cycle () =
  ...
  let load = function
    ...
    | `B -> Us.inj (Carton.Dec.v ~kind:`A b) in
  ...
  let pack =
    Carton.Dec.make { fd; mx } ~z ~allocate ~uid_ln:1
      ...
      (fun uid -> Int64.of_int (Hashtbl.find offsets uid)) in
  ...
  try
    let fiber =
      ...
      >>= fun weight ->
      let raw = Carton.Dec.make_raw ~weight in
      Carton.Dec.of_offset unix ~map pack raw ~cursor in
...
let empty_stream () =
  ...
  let decoder =
    ...
    | _ -> Alcotest.fail "Unexpected result of [decode]" in
  match Fp.decode decoder with
