...
let with_fd ~mode ~f device uid acc =
  ...
  | Ok fd -> f fd acc >>= fun acc ->
    Cstruct_append.close device fd >>= fun _ ->
    Gc.full_major () ; Lwt.return acc
...
let test_simple_use =
  ...
  with_fd ~mode:Wr ~f:(Cstruct_append.append device) device a "Hello World!" >>= fun () ->
  Gc.full_major () ;
...
let test_trunk_mode =
  ...
  with_fd ~mode:Wr ~f:(Cstruct_append.append device) device a "Hello World!" >>= fun () ->
  Gc.full_major () ;
  with_fd ~mode:Rd ~f:(fun fd _ -> Cstruct_append.map device fd ~pos:0L 12) device a Bigstringaf.empty >>= fun v ->
  Gc.full_major () ;
...
let test_two_contents =
  ...
  with_fd ~mode:Wr ~f:(Cstruct_append.append device) device a "foo" >>= fun () ->
  Gc.full_major () ;
  with_fd ~mode:Wr ~f:(Cstruct_append.append device) device b "bar" >>= fun () ->
  Gc.full_major () ;
...
let test_three_contents =
  ...
  with_fd ~mode:Wr ~f:(Cstruct_append.append device) device b "bar" >>= fun () ->
  Gc.full_major () ;
  with_fd ~mode:RdWr ~f:(fun fd str ->
      Cstruct_append.map device fd ~pos:0L 3 >>= fun v ->
      Alcotest.(check string) "contents" (Bigstringaf.to_string v) "" ; (* O_TRUNC *)
      Cstruct_append.append device fd str) device c "lol" >>= fun () ->
  Gc.full_major () ;
  let va = Cstruct_append.project device a in
  Gc.full_major () ;
  let vb = Cstruct_append.project device b in
...
let run =
  Alcotest_lwt.run "cstruct_append"
    [ "cstruct_append", [ test_simple_use
                        ; test_trunk_mode
                        ; test_two_contents
                        ; test_three_contents ] ]
