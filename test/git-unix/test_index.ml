...
let mkdir ?(perm = 0o755) path =
  Unix.mkdir (Fpath.to_string path) perm
...
module Git_status =
  struct
    ...
    let pp ppf = function
      | `Add path -> Fmt.pf ppf "A %a" Fpath.pp path

    let equal a b = match a, b with
      | `Add a, `Add b -> Fpath.equal a b
      | _, _ -> false

    let compare a b = match a, b with
      | `Add a, `Add b -> Fpath.compare a b
      | _, _ -> (-1)
...
module Git_ls_files =
  struct
    ...
    let of_output output =
      List.fold_left
        (fun acc line ->
          ...
          let lst = List.fold_left (fun acc -> function "" -> acc | x -> x :: acc) [] lst in
    ...
    let pp ppf lst =
      let pp_elt ppf (perm, hash, n, path) =
        Fmt.pf ppf "%s %a %d %a"
          ...
          Store.Hash.pp hash n Fpath.pp path in
    ...
    let equal a b =
      List.for_all2 equal_elt
        (List.sort compare_elt a)
...
module Git_ls_tree =
  struct
    ...
    let pp ppf lst =
      let pp_kind : kind Fmt.t = fun ppf -> function
        | `Commit -> Fmt.string ppf "commit"
        | `Tag -> Fmt.string ppf "tag"
        | `Tree -> Fmt.string ppf "tree"
        | `Blob -> Fmt.string ppf "blob" in
      let pp_elt ppf (perm, kind, hash, path) =
        Fmt.pf ppf "%s %a %a %a" (Store.Value.Tree.string_of_perm perm) pp_kind kind Store.Hash.pp hash Fpath.pp path in
      Fmt.Dump.list pp_elt ppf lst
...
let make_empty_index t fs =
  ...
  Index.IO.store ~root:(Store.dotgit t) fs ~raw [] >>?= fun () ->
  let command = Fmt.strf "git --git-dir=%a status --porcelain" Fpath.pp (Store.dotgit t) in
...
let update_index_add_one_file t fs path =
  ...
  let raw  = Cstruct.create 0x800 in
  Index.IO.load ~root:(Store.dotgit t) ~dtmp fs >>?= fun (entries, _) ->
  let hash = hash_of_file Fpath.(Store.root t // path) in
  let entry = Index.entry_of_stats hash path (stat Fpath.(Store.root t // path)) in
...
let update_index_add_one_symlink t fs path =
  ...
  let entry = Index.entry_of_stats hash path (lstat Fpath.(Store.root t // path)) in
  Index.IO.store ~root:(Store.dotgit t) fs ~raw (entry :: entries)
...
let update_index_remove_one_file t fs path =
  ...
  let raw  = Cstruct.create 0x800 in
  Index.IO.load ~root:(Store.dotgit t) ~dtmp fs >>?= fun (entries, _) ->
  let entries = List.filter (fun entry -> not (Fpath.equal path entry.Entry.path)) entries in
...
let test000 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    (Fmt.strf "make directory %a" Fpath.pp root)
    `Quick
    (fun () ->
      try mkdir root
...
let test002 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    "touch should-be-empty"
    `Quick
    (fun () ->
      try touch Fpath.(root / "should-be-empty")
      with exn -> Alcotest.failf "Retrieve an error: %s" (Printexc.to_string exn))

let test003 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    "git update-index with --add"
    `Quick
    (run
       ~root
       ~pp_error:Index.pp_error
     @@ fun t -> update_index_add_one_file t fs (Fpath.v "should-be-empty") >>?= fun () ->
                 let command = Fmt.strf "git --git-dir=%a status --porcelain" Fpath.pp (Store.dotgit t) in
                 let output = Test_data.output_of_command command in
                 let status = Git_status.of_output output in
                 Alcotest.(check git_status) "should-be-empty" [ `Add Fpath.(v "should-be-empty") ] status;
                 Lwt.return (Ok ()))

let test004 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    "write-tree should-be-empty"
    `Quick
    (run
       ~root
     ...
     @@ fun t ->
        Index.Write.write t fs >>?= fun hash ->
        assert_key_equal "write-tree" hash (Store.Hash.of_hex "7bb943559a305bdd6bdee2cef6e5df2413c3d30a");
        Lwt.return (Ok ()))
...
let test005 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    ...
    `Quick
    (fun () ->
      try remove Fpath.(root / "should-be-empty")
      with exn -> Alcotest.failf "Retrieve an error: %s" (Printexc.to_string exn))

    (* XXX(dinosaure): at this stage, `git` does not check status of repository. From what I know, it said:
       A should-be-empty
       D should-be-empty
...
let test006 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    "git update-index with --remove"
    `Quick
    (run
       ~root
       ~pp_error:Index.pp_error
     @@ fun t -> update_index_remove_one_file t fs (Fpath.v "should-be-empty") >>?= fun () ->
                 let command = Fmt.strf "git --git-dir=%a status --porcelain" Fpath.pp (Store.dotgit t) in
                 let output = Test_data.output_of_command command in
                 let status = Git_status.of_output output in
...
let test007 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    ...
    (run
     ...
     @@ fun t ->
        Index.Write.write t fs >>?= fun hash ->
        assert_key_equal "write-tree" hash (Store.Hash.of_hex "4b825dc642cb6eb9a060e54bf8d69288fbee4904");
        Lwt.return (Ok ()))

let test008 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    "make directories, files and links"
    `Quick
    (fun () ->
      try
        let open Fpath in
...
let test009 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    "git update-index --add various types of objects"
    `Quick
    (run
       ~root
       ~pp_error:Index.pp_error
     @@ fun t ->
        ...
        update_index_add_one_file t fs (v "path0") >>?= fun () ->
        update_index_add_one_symlink t fs (v "path0sym") >>?= fun () ->
        update_index_add_one_file t fs (v "path2" / "file2") >>?= fun () ->
        update_index_add_one_symlink t fs (v "path2" / "file2sym") >>?= fun () ->
        update_index_add_one_file t fs (v "path3" / "file3") >>?= fun () ->
        update_index_add_one_symlink t fs (v "path3" / "file3sym") >>?= fun () ->
        update_index_add_one_file t fs (v "path3" / "subp3" / "file3") >>?= fun () ->
        update_index_add_one_symlink t fs (v "path3" / "subp3" / "file3sym") >>?= fun () ->
        let command = Fmt.strf "git --git-dir=%a status --porcelain" Fpath.pp (Store.dotgit t) in
        let output = Test_data.output_of_command command in
        let status = Git_status.of_output output in
        Alcotest.(check git_status) "various kind of objects"
          [ `Add (v "path0")
          ; `Add (v "path0sym")
          ; `Add (v "path2" / "file2")
          ...
          ; `Add (v "path3" / "subp3" / "file3sym") ] status;
        Lwt.return (Ok ()))
...
let test010 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    ...
    (run
       ~root
       ~pp_error:(fun _ppf _ -> assert false)
     @@ fun t ->
      let open Fpath in

      let command = Fmt.strf "git --git-dir=%a ls-files --stage" Fpath.pp (Store.dotgit t) in
      let output = Test_data.output_of_command command in
      let ls_files = Git_ls_files.of_output output in
      Alcotest.(check git_ls_files) "git ls-files --stage"
        [ `Normal, Store.Hash.of_hex "f87290f8eb2cbbea7857214459a0739927eab154", 0, v "path0"
        ; `Link,   Store.Hash.of_hex "15a98433ae33114b085f3eb3bb03b832b3180a01", 0, v "path0sym"
        ; `Normal, Store.Hash.of_hex "3feff949ed00a62d9f7af97c15cd8a30595e7ac7", 0, v "path2" / "file2"
        ; `Link,   Store.Hash.of_hex "d8ce161addc5173867a3c3c730924388daedbc38", 0, v "path2" / "file2sym"
        ; `Normal, Store.Hash.of_hex "0aa34cae68d0878578ad119c86ca2b5ed5b28376", 0, v "path3" / "file3"
        ; `Link,   Store.Hash.of_hex "8599103969b43aff7e430efea79ca4636466794f", 0, v "path3" / "file3sym"
        ; `Normal, Store.Hash.of_hex "00fb5908cb97c2564a9783c0c64087333b3b464f", 0, v "path3" / "subp3" / "file3"
        ; `Link,   Store.Hash.of_hex "6649a1ebe9e9f1c553b66f5a6e74136a07ccc57c", 0, v "path3" / "subp3" / "file3sym" ]
        ls_files;
      Lwt.return (Ok ()))
...
let test011 root fs : unit Alcotest.test_case =
  Alcotest.test_case
    ...
    (run
     ...
     @@ fun t ->
        Index.Write.write t fs >>?= fun hash ->
        assert_key_equal "write-tree" hash (Store.Hash.of_hex "087704a96baf1c2d1c869a8b084481e121c88b5b");
        Lwt.return (Ok ()))
...
let test012 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    ...
    (run
       ~root
       ~pp_error:Store.pp_error
     @@ fun t ->
        let root_tree = Store.Hash.of_hex "087704a96baf1c2d1c869a8b084481e121c88b5b" in
        let command = Fmt.strf "git --git-dir=%a ls-tree %a" Fpath.pp (Store.dotgit t) Store.Hash.pp root_tree in
        let output = Test_data.output_of_command command in
        let ls_tree = Git_ls_tree.of_output output in

        Store.read t root_tree >>?= function
        | Store.Value.Tree tree ->
           let lst = List.map (function
                         ...
                         | { Store.Value.Tree.perm; node; name; } ->
                            (perm, `Blob, node, Fpath.v name)) (Store.Value.Tree.to_list tree) in

           Alcotest.(check git_ls_tree) "git ls-tree" lst ls_tree;
           Lwt.return (Ok ())
        | value -> Alcotest.failf "Expected a tree git object from %a and have: %a"
                     Store.Hash.pp root_tree Store.Value.pp value)

let test013 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    "git ls-tree recursive"
    `Quick
    (run
       ~root
       ~pp_error:Store.pp_error
     @@ fun t ->
        ...
        let root_tree = Store.Hash.of_hex "087704a96baf1c2d1c869a8b084481e121c88b5b" in
        let command = Fmt.strf "git --git-dir=%a ls-tree -r %a" Fpath.pp (Store.dotgit t) Store.Hash.pp root_tree in
        let output = Test_data.output_of_command command in
        let ls_tree = Git_ls_tree.of_output output in
        ...
        Store.fold t
          (fun acc ?name ~length:_ hash value -> match name, value with
            | Some path, Store.Value.Tree tree ->
               let () = List.iter (fun { Store.Value.Tree.perm; name; _ } -> Hashtbl.add perms Fpath.(path / name) perm)
                          (Store.Value.Tree.to_list tree) in
               Lwt.return acc
            | Some path, Store.Value.Blob _   -> Lwt.return ((Hashtbl.find perms path, `Blob,   hash, path) :: acc)
            | Some path, Store.Value.Commit _ -> Lwt.return ((Hashtbl.find perms path, `Commit, hash, path) :: acc)
            | Some path, Store.Value.Tag _    -> Lwt.return ((Hashtbl.find perms path, `Tag,    hash, path) :: acc)
            | None, _ -> Alcotest.failf "Expected only named git object")
          ~path:Fpath.(v ".") [] root_tree >>= fun lst ->
        Alcotest.(check git_ls_tree) "git ls-tree -r" lst ls_tree;
        Lwt.return (Ok ()))

let test014 root _ : unit Alcotest.test_case =
  Alcotest.test_case
    "git ls-tree recursive with tree"
    `Quick
    (run
       ~root
       ~pp_error:Store.pp_error
     @@ fun t ->
        ...
        let root_tree = Store.Hash.of_hex "087704a96baf1c2d1c869a8b084481e121c88b5b" in
        let command = Fmt.strf "git --git-dir=%a ls-tree -r -t %a" Fpath.pp (Store.dotgit t) Store.Hash.pp root_tree in
        let output = Test_data.output_of_command command in
        let ls_tree = Git_ls_tree.of_output output in
        ...
        Store.fold t
          (fun acc ?name ~length:_ hash value -> match name, value with
            ...
            | None, _ -> Alcotest.failf "Expected only named git object")
          ~path:Fpath.(v ".") [] root_tree >>= fun lst ->
        Alcotest.(check git_ls_tree) "git ls-tree -r" lst ((`Dir, `Tree, root_tree, Fpath.v ".") :: ls_tree);
        Lwt.return (Ok ()))
  ...
  Alcotest.test_case
    ...
    (run
     ...
     @@ fun t ->
        ...
        Index.Read.from_tree t root >>= fun entries ->

        Fmt.(pf stderr)
          "We will write: %a.\n" Entry.pp_index entries;

        Index.IO.store ~root:(Store.dotgit t) fs ~raw entries >>?= fun () ->
        Index.Write.write t fs >>?= fun hash ->
