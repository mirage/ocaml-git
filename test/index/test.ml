...
let blob_of_path path =
  if (Unix.lstat (Fpath.to_string path)).Unix.st_kind = Unix.S_LNK
  then
...
let tree_of_children tbl children =
  ...
  let entry = function
    ...
    | None -> Alcotest.failf "%a does not exist" Fpath.pp path in
    ...
    let fiber =
      ...
      let f v children _ =
        ...
        | `Root -> (
            ...
            | Error err ->
                Alcotest.failf "store: %a" Git_unix.Store.pp_error err) in
      fold ~f (Digestif.SHA1.digest_string "") t in
